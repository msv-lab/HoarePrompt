...
Your Task:
Precondition: `numbers` has fewer elements, `num` has a new value, `total` and `count` may be adjusted
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` has fewer elements, `num` has a new value, `total` is adjusted, and `count` is increased by 1
else part: `numbers` has fewer elements, `num` has a new value, `total` and `count` may be adjusted
