...
Your Task:
Precondition: `numbers` is modified due to popping an element, and `num` is assigned the popped value from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is modified due to popping an element, `num` is assigned the popped value from the list, `total` is increased by `num`, `count` is incremented by an unknown value
else part: `numbers` is modified due to popping an element, and `num` is assigned the popped value from the list
