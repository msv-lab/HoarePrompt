...
Your Task:
Initial State: `numbers` is a list of integers with one less element, `total` is the sum of all popped integers, `count` is 0, `num` is the integer popped from the list
```
count += 1
