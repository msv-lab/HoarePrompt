...
Your Task:
Precondition: numbers is a list of integers with one less element, total is 0, count is 0, and `num` is the integer that was removed from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: numbers is a list of integers with one less element, total is the sum of all elements in the list, count is 1 more than previous value, and `num` is the integer that was removed from the list
else part: numbers is a list of integers with one less element, total is 0, count is 0, and `num` is the integer that was removed from the list, 'Skipping non-positive number:', num is printed
