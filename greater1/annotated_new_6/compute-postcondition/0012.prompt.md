...
Your Task:
Precondition: `numbers` is a list of integers with at least one element less; `num` is the popped integer value
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with at least one element less; `num` is the popped integer value, `total` is increased by the value of `num`, `count` is incremented by 1
else part: `numbers` is a list of integers with at least one element less
