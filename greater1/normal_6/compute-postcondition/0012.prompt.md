...
Your Task:
Precondition: `numbers` has one less element, `total` is the sum of all elements in `numbers` that are greater than 1, `count` is the number of elements in `numbers` that are greater than 1, and `num` is the last popped integer
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` has one less element, `total` is the sum of all elements in `numbers` that are greater than 1 plus the value of `num`, `count` is incremented by 1, and `num` is the last popped integer
else part: `numbers` has one less element, `total` is the sum of all elements in `numbers` that are greater than 1, `count` is the number of elements in `numbers` that are greater than 1, and `num` is the last popped integer
