...
Your Task:
Precondition: `numbers` is a list of integers with one less element, `total` and `count` are updated based on the value of `num`
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with one less element, `total` is updated based on the value of `num`, `count` is incremented by 1
else part: `numbers` is a list of integers with one less element, `total` and `count` are updated based on the value of `num`
