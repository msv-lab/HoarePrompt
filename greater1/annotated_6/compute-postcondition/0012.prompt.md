...
Your Task:
Precondition: `numbers` is a list of integers with at least one element (one less element); `num` is the popped integer
Program Fragment:
```
if num > 1:
    total += num
...
else part: `numbers` is a list of integers with at least one element; `num` is the popped integer
