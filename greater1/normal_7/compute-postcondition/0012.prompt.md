...
Your Task:
Precondition: `num` is assigned the value popped from the `numbers` list, `numbers` list is updated after popping the value
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `num` is added to `total`, `num` is assigned the value popped from the `numbers` list, `numbers` list is updated after popping the value, `count` is increased by 1
else part: `num` is assigned the value popped from the `numbers` list, `numbers` list is updated after popping the value
