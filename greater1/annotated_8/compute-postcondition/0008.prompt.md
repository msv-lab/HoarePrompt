...
Your Task:
Initial State: `numbers` is a list of integers with the last element removed (not empty), `total` is increased by the value of `num`, `count` is increased by 1, and the last element `num` is greater than 1
```
num = numbers.pop()
