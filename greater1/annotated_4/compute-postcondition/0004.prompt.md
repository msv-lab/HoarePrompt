...
Your Task:
Initial State: `numbers` is a list of integers with the last element removed, `total` is the sum of the removed integer, `count` is 0, `num` is the removed integer
```
count += 1
