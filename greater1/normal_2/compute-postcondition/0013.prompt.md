...
Your Task:
State at the end of the previous iteration: *num is a number. If num is greater than 1, total is increased by num and count is increased by 1. If num is less than or equal to 1, the program skips the non-positive number and does not update total or count.
```
while numbers:
    # the loop body is omit
```
