...
Your Task:
Initial State: `numbers` is a list of integers with at least one element left, `num` is the last element that was removed from the list, `total` has been incremented by `num`
```
count += 1
