...
Your Task:
Initial State: `numbers` is a list of integers with one less element, `total` is updated with the addition of `num`, `count` is 0, and `num` is the last element removed from the list
```
count += 1
