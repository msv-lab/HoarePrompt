...
Your Task:
Precondition: `numbers` has one less element, `num` is the last element that was removed from the list
Program Fragment:
```
if num > 1:
    total += num
...
else part: `numbers` has one less element, `num` is the last element that was removed from the list
