...
Your Task:
Precondition: `numbers` is a list of integers with reduced size, `num` is the popped integer, `total` and `count` are adjusted accordingly
Program Fragment:
```
if num > 1:
    total += num
...
else part: `numbers` is a list of integers with reduced size, `num` is the popped integer, `total` and `count` are adjusted accordingly
