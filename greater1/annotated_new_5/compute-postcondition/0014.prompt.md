...
Your Task:
Initial State: `numbers` is a list of integers with at least one element remaining, `num` is the popped integer from the list, `total`, and `count` are adjusted accordingly based on the value of `num`
```
num = numbers.pop()
