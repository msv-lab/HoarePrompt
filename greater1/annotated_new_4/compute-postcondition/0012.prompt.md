...
Your Task:
Precondition: `numbers` is a list of integers with at least one element remaining, `num` holds the removed integer, `total` and `count` are adjusted accordingly
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with at least one element remaining, `num` holds the removed integer, `total` is incremented by `num`, `count` is increased by 1
else part: `numbers` is a list of integers with at least one element remaining, `num` holds the removed integer, `total` and `count` are adjusted accordingly
