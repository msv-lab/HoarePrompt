...
Your Task:
Initial State: `numbers` is a list of integers with one less element, `total` and `count` may have been updated based on the last popped `num`, `num` holds the popped value, `total` is updated with the addition of `num`
```
count += 1
