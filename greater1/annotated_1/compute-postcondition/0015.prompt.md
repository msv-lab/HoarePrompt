...
Your Task:
Initial State: `numbers` is a list of integers with at least one less element, `total` is either increased by the value of the removed element, `count` is either increased by 1, or the last element is removed from the list
```
total += num
