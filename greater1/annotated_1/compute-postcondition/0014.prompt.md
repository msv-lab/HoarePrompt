...
Your Task:
Initial State: `numbers` is a list of integers with at least one less element. If the last element of `numbers` is greater than 1, `total` is increased by the value of the last element and `count` is increased by 1. If the last element of `numbers` is less than or equal to 1, the last element is removed from the list.
```
num = numbers.pop()
