...
Your Task:
Precondition: `numbers` is a list of integers with at least one less element; `num` is the last element popped from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with at least one less element, `total` is incremented by the value of the last element popped from the list, `count` is incremented by 1
else part: `numbers` is a list of integers with at least one less element; `num` is the last element popped from the list
