...
Your Task:
State at the end of the previous iteration: *If the last removed `num` is greater than 1, `total` is incremented by the value of `num` and `count` is increased by 1. If the last removed `num` is not greater than 1, the message 'Skipping non-positive number: num' is printed. In both cases, `total` and `count` are updated according to the condition met by `num`.
```
while numbers:
    # the loop body is omit
```
