...
Your Task:
Initial State: `total` is updated with the addition of `num`, `count` remains the same, `numbers` still has one less element, and `num` is the last element of the original list
```
count += 1
