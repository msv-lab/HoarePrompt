...
```
Output state after the execution of the try statement: `input_string` is a string, `value` is either an integer or an error, `result` is the result of 100 divided by `value` if `value` is an integer, or a division by zero error if `value` is an error
Output state after the execution of the except statement(s): 
except_1: `result` is 'Invalid input'
except_2: `result` is 'Cannot divide by zero'
