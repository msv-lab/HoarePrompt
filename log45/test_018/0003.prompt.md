...
Initial State: filename is a string representing a valid file path.
Code for the try except block:
```
try:
...
```
Output state after the execution of the try statement: `filename` is a string representing a valid file path; `file` is either the opened file object or an error if the file could not be opened; `content` is the content of the file if it was successfully read.
Output state after the execution of the except statement(s): 
except_1: `content` is an empty string
