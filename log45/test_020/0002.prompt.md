...
Initial State: numbers is a list, index is an integer.
Code for the try except block:
```
try:
    value = numbers[index]
...
```
Output state after the execution of the try statement: `numbers` is a list, `index` is an integer, `value` is assigned the value at the specified index from the list
Output state after the execution of the except statement(s): 
except_1: `value` is `None`
