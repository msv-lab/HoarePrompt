...
Your Task:
Initial State: `total` is equal to `matrix[0][0] + matrix[0][n - 1]`, `matrix` is a 2D list of integers representing a square matrix of dimensions n×n where n must be greater than 1, `i` is 1
```
total += matrix[i][i]
