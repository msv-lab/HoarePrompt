...
Your Task:
State at the end of the previous iteration: `total` is equal to `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2]`, `matrix` is a 2D list of integers representing a square matrix of dimensions n×n where n must be greater than 1, `i` is 1
```
for i in range(n):
    # the loop body is omit
```
