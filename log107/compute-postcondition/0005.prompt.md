...
Your Task:
State at the end of the previous iteration: `total` is equal to `matrix[0][0] + matrix[0][n - 1]`, `matrix` is a 2D list of integers representing a square matrix of dimensions n×n where n must be greater than 0, `i` is 0.
```
for i in range(n):
    # the loop body is omit
```
