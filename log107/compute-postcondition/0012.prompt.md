...
Your Task:
Initial State: `total` is equal to the sum of the elements on the main diagonal and the secondary diagonal of `matrix`, `matrix` is a 2D list of integers representing a square matrix of dimension n×n where n is a non-negative integer.
```
return total
