...
Your Task:
State before the loop starts: `matrix` is a 2D list of integers representing a square matrix of dimension n×n, where `n` is equal to the number of rows in `matrix`; `total` is 0.
```
for i in range(n):
    # the loop body is omit
```
