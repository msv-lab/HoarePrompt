...
#Overall this is what the function does: The function accepts a parameter x, returns x minus 1 if x is less than 0; returns x increased by 1 if x is greater than 0; returns x unchanged if x is equal to 0.

```

Return Postconditions: Case_1: The program returns x minus 1, where x is an integer less than 0

Case_2: The program returns the value of x, which could be increased by 1 if x is greater than 0, decreased by 1 if x is less than 0, or remain unchanged if x is equal to 0.
