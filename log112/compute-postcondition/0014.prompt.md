...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer less than 10^18, `k` is a positive integer greater than or equal to 3, `dp[n, k]` is now 9 + 9 * 10 * (10^(k - 2) - 10), `i` is 2
```
for i in range(1, k):
    # the loop body is omit
```
