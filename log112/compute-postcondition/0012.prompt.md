...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer less than \(10^{18}\), `k` is a positive integer such that 1 <= `k` <= 10, `k` is greater than 0, `dp[n, k]` is now 9 * (10
```
for i in range(1, k):
    # the loop body is omit
```
