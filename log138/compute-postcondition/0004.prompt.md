...
Your Task:
Initial State: `balances` is a list of two integers where the first element is A - amount and the second element is B; the balance of Account A is now A - amount
```
B = B + amount
