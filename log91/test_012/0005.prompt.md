...
Your Task:
Precondition: *`x`, `y`, and `z` are integers. If `x` is greater than 0, `y` is increased by `x` while `z` remains unchanged. If `x` is less than or equal to 0, `y` remains unchanged and `z` also remains unchanged.
Program Fragment:
```
if y < 0:

```
if part: `x`, `y` and `z` are integers; `y` is less than 0; `z` remains unchanged; `z` has been decreased by `y`.
else part: None
