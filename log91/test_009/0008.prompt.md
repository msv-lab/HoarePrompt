...
```
if part: `x` is an integer greater than 0; `y` is 'positive'
else part: *`x` is an integer less than or equal to 0. If `x` is 0, then `y` is set to 'zero'. If `x` is less than 0, then `y` is set to 'negative'.
