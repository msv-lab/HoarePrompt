...
Initial State: data is a dictionary, key is a string.
Code for the try except block:
```
try:
    value = data[key]
...
```
Output state after the execution of the try statement: `data` is a dictionary, `key` is a string, `value` is the value associated with `key` in `data`.
Output state after the execution of the except statement(s): 
except_1: `value` is 'Key not found'
