...
Your Task:
Precondition: *`a`, `b`, and `c` are integers. `a` is greater than `b`.
Program Fragment:
```
if b > c:
else:
...
```
if part: `a`, `b`, and `c` are integers; `a` is greater than `b`, and `b` is greater than `c`; `x` is equal to `a - c`.
else part: *`a`, `b`, and `c` are integers. If `c` is greater than `a`, then `x` is a positive integer resulting from `c - b`. Otherwise, `x` is equal to `a + b`.
