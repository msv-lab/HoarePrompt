...
```
if part: Postcondition: ***`a`, `b`, and `c` are integers. If `b` is greater than `c`, then `x` is equal to `a - c`. If `c` is greater than `a`, then `x` is a positive integer resulting from `c - b`. Otherwise, `x` is equal to `a + b.```
else part: `a`, `b`, and `c` are integers; `a` is less than or equal to `b`; `x` is `b + c`
