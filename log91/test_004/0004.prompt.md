...
Your Task:
State at the end of the previous iteration: `total` is the last value of `numbers` plus the last element of `numbers`; `numbers` is the original list with one less element.
```
while numbers:
    # the loop body is omit
```
