...
Your Task:
Initial State: `total` is the last value of `numbers` plus the last element of `numbers`; `numbers` is the original list with one less element and for the loop to execute again, `numbers` must contain at least one element
```
total += numbers.pop()
