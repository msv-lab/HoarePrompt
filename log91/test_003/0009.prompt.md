...
Output State after the loop executes 1 times: `counter` is an integer greater than or equal to 0, `total` is equal to the original value of `counter`
Output State after the loop executes 2 times: `counter` is an integer greater than 0 after being decremented by 1; `total` is equal to `2 * counter` before the decrement.
Output State after the loop executes 3 times: `counter` is an integer greater than 0 (now decreased by 1), `total` remains unchanged
