...
Your Task:
State at the end of the previous iteration: `counter` is an integer greater than 0 after being decremented by 1; `total` is equal to `2 * counter` before the decrement.
```
while counter > 0:
    # the loop body is omit
```
