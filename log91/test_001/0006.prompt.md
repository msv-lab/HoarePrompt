...
Initial State: n is an integer, total is 0
Code of the loop:
```
for i in range(n):
    total += i
...
```
Output State after the loop executes 1 times: `total` is 0, `i` is 0
Output State after the loop executes 2 times: `total` is 1, `i` is 1, and for the loop to execute a second time `n` must be greater than 1.
Output State after the loop executes 3 times: `total` is 3, `i` is 2
