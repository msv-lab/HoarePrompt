...
Your Task:
Initial State: `inner_counter` is greater than 0, `outer_counter` is less than or equal to its original value minus 1, `total` is increased by the original value of `inner_counter
```
inner_counter -= 1
