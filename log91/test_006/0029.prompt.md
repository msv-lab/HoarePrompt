...
Your Task:
Initial State: `inner_counter` is adjusted to be greater than 0, `outer_counter` is less than or equal to its original value minus 1, `total` remains increased by the original value of `inner_counter`
```
total += inner_counter
