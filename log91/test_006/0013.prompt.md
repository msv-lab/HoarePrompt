...
Your Task:
State right before the while loop: `inner_counter` is 0, `outer_counter` is now less than or equal to its original value minus 1 and for the loop to execute again `outer_counter` must be greater than 0
```
