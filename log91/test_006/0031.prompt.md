...
Your Task:
State at the end of the previous iteration: `inner_counter` is now one less than its original value; `outer_counter` is less than or equal to its original value minus 1; `total` is increased by the original value of `inner_counter.
```
