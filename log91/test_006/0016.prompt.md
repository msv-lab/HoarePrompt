...
Your Task:
State at the end of the previous iteration: `inner_counter` is greater than or equal to 0, `outer_counter` is now less than or equal to its original value minus 1, and `total` is increased by `inner_counter.
```
