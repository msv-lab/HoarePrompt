...
Your Task:
Initial State: `inner_counter` is 0, `total` is the sum of all integers from the original value of `inner_counter` down to 1, `outer_counter` is less than or equal to its original value minus 1.
```
outer_counter -= 1
