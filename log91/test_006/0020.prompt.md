...
Your Task:
Initial State: `inner_counter` is decreased by 1, for the loop to execute again `inner_counter` must currently be greater than 0, so it should have been at least 2 before the last iteration
```
total += inner_counter
