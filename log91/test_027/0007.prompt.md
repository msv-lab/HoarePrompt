...
```
if part: `x` is an integer, `y` is an integer, and `x` is equal to `y`; `z` is equal to `2 * x`
else part: `x` is an integer, `y` is an integer, `x` is not equal to `y`, `x` is less than or equal to `y`, and `z` is a positive integer.
