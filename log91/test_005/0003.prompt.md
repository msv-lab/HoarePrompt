...
Your Task:
State at the end of the previous iteration: `total` is now `num`, `matrix` is a 2D list of integers, `row` is the first row in the matrix containing at least 1 integer
```
for num in row:
    # the loop body is omit
```
