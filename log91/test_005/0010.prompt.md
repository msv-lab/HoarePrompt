...
Your Task:
Initial State: `total` is equal to the sum of all integers in the first row of `matrix`, `row` is the second row of `matrix` with at least 2 elements, for the loop to execute a second time `num` is the second element in `row`
```
total += num
