...
Your Task:
Initial State: `total` is equal to the sum of all integers in the first row of `matrix` plus the sum of all integers in the second row of `matrix` plus `num`, for the loop to execute a second time `row` must have at least 3 elements, and `num` is the third element in `row`.
```
total += num
