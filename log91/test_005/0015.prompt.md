...
Initial State: `total` is equal to the sum of all integers in the first row of `matrix`, `row` is the second row of `matrix`, for the loop to execute a second time `matrix` must have at least 2 rows
Code of the loop:
```
for num in row:
    total += num
...
```
Output State after the loop executes 1 times: `total` is equal to the sum of all integers in the first row of `matrix` plus `num`, `row` is the second row of `matrix` with at least 2 elements, `num` is the second element in `row`
Output State after the loop executes 2 times: `total` is equal to the sum of all integers in the first row of `matrix` plus `2 * num`, `row` is the second row of `matrix`, `num` is the third element in `row
Output State after the loop executes 3 times: `total` is equal to the sum of all integers in the first row of `matrix` plus `2 * num`, `row` is the second row of `matrix`, `num` is the fourth element in `row`; `total` is increased by `num`.
