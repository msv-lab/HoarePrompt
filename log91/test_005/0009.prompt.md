...
Your Task:
State before the loop starts: `total` is equal to the sum of all integers in the first row of `matrix`, `row` is the second row of `matrix`, for the loop to execute a second time `matrix` must have at least 2 rows
```
for num in row:
    # the loop body is omit
```
