...
Your Task:
Initial State: `total` is 0, for the loop to execute the first time `matrix` is a 2D list of integers with at least 1 row, `row` is the first row in the matrix containing at least 1 integer
```
total += num
