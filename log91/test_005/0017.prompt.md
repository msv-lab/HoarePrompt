...
Your Task:
State before the loop starts: `total` is equal to the sum of all integers in the first row of `matrix` plus the sum of all integers in the second row of `matrix`, `row` is the third row of `matrix`, and `num` is the last element in `row`.
```
for num in row:
    # the loop body is omit
```
