...
Your Task:
Initial State: `total` is equal to the sum of all integers in the first row of `matrix` plus `num`, `row` is the second row of `matrix` with at least 3 elements, `num` is the third element in `row`
```
total += num
