...
Your Task:
State before the loop starts: `total` is 0, for the loop to execute the first time `matrix` is a 2D list of integers with at least 1 row, `row` is the first row in the matrix
```
for num in row:
    # the loop body is omit
```
