...
Your Task:
Initial State: `total` is equal to the sum of all integers in the first row of `matrix` plus the sum of all integers in the second row of `matrix` plus `num`, for the loop to execute a second time `row` must be a list with at least 2 elements, and `num` is the second element of `row`
```
total += num
