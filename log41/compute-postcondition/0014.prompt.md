...
Initial State: `n` is a non-negative integer, `prev` is 0, `curr` is 1, `i` is 0
Code of the loop:
```
while i <= n:
...
Output State after the loop executes 1 times: `n` is a non-negative integer, `prev` is 1, `curr` is 0, `i` is -1
Output State after the loop executes 2 times: `n` is a non-negative integer, `prev` is either 0 or 1, `curr` is 0 or 1, `i` is less than or equal to `n, `i` is decreased by 1
Output State after the loop executes 3 times: `n` is a non-negative integer, `prev` and `curr` are either 0 or 1, `i` is less than or equal to `n` and `i` is decreased by 1
