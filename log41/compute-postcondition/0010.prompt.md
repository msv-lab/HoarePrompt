...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer, `prev` is either 0 or 1, `curr` is 0 or 1, `i` is less than or equal to `n, `i` is decreased by 1
```
while i <= n:
    # the loop body is omit
```
