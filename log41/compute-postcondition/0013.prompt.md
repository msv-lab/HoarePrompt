...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer, `prev` and `curr` are either 0 or 1, `i` is less than or equal to `n` and `i` is decreased by 1
```
while i <= n:
    # the loop body is omit
```
