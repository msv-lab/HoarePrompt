...
Your Task:
Precondition: `l` and `r` are integers such that 1 <= l <= r < 10^18, and `k` is an integer such that 1 <= k <= 10; `t`, `s`, `x` are input integers.
Program Fragment:
```
...
```
if part: `l` and `r` are integers such that 1 <= l <= r < 10^18, `k` is an integer such that 1 <= k <= 10, `t`, `s`, `x` are input integers, the current value of `(x - t) % s` is either 0 or 1, and the program printed 'YES'.
else part: `l` and `r` are integers such that 1 <= `l` <= `r` < 10^18, `k` is an integer such that 1 <= `k` <= 10; `t`, `s`, `x` are input integers. The value of `(x - t) % s` is not equal to 0 and is not equal to 1. Printed output: 'NO'
