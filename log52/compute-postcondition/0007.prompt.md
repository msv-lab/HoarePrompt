...
#Overall this is what the function does: The function accepts a parameter x, returns the updated value of x after either increasing by 1 if x was greater than 0, or decreasing by 1 if x was less than or equal to 0.

```

Return Postconditions: The program returns the updated value of integer `x` after either increasing by 1 if `x` was greater than 0, or decreasing by 1 if `x` was less than or equal to 0
