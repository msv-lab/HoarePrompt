...
def func_1(x):
    if (x > 0) :
        return x + 1
        #The program returns x plus 1, where x is an integer greater than 0
    else :
        return x - 1
        #The program returns x minus 1, where x is an integer less than or equal to 0
#Overall this is what the function does: The function accepts a parameter x, returns x plus 1 if x is greater than 0; if x is less than or equal to 0, it returns x minus 1.

```

Return Postconditions: Case_1: The program returns x plus 1, where x is an integer greater than 0
