...
Your Task:
Precondition: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `completed` is the total count of 'complete' tasks in `tasks' (including the current one), `pending` is the total count of 'pending' tasks in `tasks' (including the current one)
Program Fragment:
```
if task == 'pending':
    pending += 1
...
```
if part: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `completed` is the total count of 'complete' tasks in `tasks' (including the current one), `pending` is increased by 1.
else part: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `completed` is the total count of 'complete' tasks in `tasks' (including the current one), `pending` is the total count of 'pending' tasks in `tasks' (including the current one); 'Unknown task' is printed.
