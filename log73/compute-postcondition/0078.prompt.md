...
Your Task:
State at the end of the previous iteration: `tasks` is a list of strings containing tasks, `running` is the result of check_status(), `completed` is the total count of 'complete' tasks in `tasks`, `pending` is the total count of 'pending' tasks in `tasks`.
```
while running:
    # the loop body is omit
```
