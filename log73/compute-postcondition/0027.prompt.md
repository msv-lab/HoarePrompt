...
Your Task:
State before the loop starts: `tasks` is a list of strings containing tasks that are either 'complete' or 'pending'; `running` is True; `completed` is the count of 'complete' tasks in the list; `pending` is the count of 'pending' tasks in the list.
```
for task in tasks:
    # the loop body is omit
```
