...
Your Task:
Precondition: `tasks` is a list of strings containing at least 1 task, `running` is true, `completed` is the total count of 'complete' tasks in `tasks', `pending` is the total count of 'pending' tasks in `tasks', `task` is the first task in the list
Program Fragment:
```
if task == 'complete':
    completed += 1
...
```
if part: `tasks` is a list of strings containing at least 1 task, `running` is true, `completed` is the total count of 'complete' tasks in `tasks` increased by 1, `pending` is the total count of 'pending' tasks in `tasks', `task` is the first task in the list and the value of `task` is 'complete'.
else part: *`tasks` is a list of strings containing at least 1 task, `running` is true, `completed` is the total count of 'complete' tasks in `tasks', and `pending` is the total count of 'pending' tasks in `tasks'. If the first task in `tasks` is 'pending', `pending` is increased by 1. Otherwise, if the first task is not 'pending' and not 'complete', 'Unknown task' is printed, leaving `pending` unchanged.
