...
Your Task:
Precondition: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `task` is the second task in the list
Program Fragment:
```
if task == 'complete':
    completed += 1
...
```
if part: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `task` is the second task in the list, and `completed` is increased by 1.
else part: *`tasks` is a list of strings containing at least 2 tasks, `running` is true, and `task` is the second task in the list. If `task` is 'pending', then `pending` is increased by 1. If `task` is not 'pending', the program prints 'Unknown task'.
