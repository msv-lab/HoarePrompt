...
Your Task:
State at the end of the previous iteration: *`tasks` is a list of strings with at least 2 tasks, `running` is True, and `completed` is incremented by 1. If `task` is 'complete', `completed` increases by 1, while `pending` remains 0. If `task` is 'pending', `pending` is incremented by 1; otherwise, `pending` remains 0 and 'Unknown task' is printed.
```
for task in tasks:
    # the loop body is omit
```
