...
Your Task:
State at the end of the previous iteration: *`tasks` is a list of strings containing at least 2 tasks and `running` is True. If `task` is 'complete', `completed` is increased by 1. If `task` is 'pending', `pending` is increased by 1. If `task` is neither 'complete' nor 'pending', 'Unknown task' is printed to the output.
```
for task in tasks:
    # the loop body is omit
```
