...
```
if part: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `task` is the second task in the list, and `pending` is increased by 1
else part: `tasks` is a list of strings containing at least 2 tasks, `running` is true, `task` is the second task in the list; 'Unknown task' is printed.
