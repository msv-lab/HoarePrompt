...
Your Task:
State before the loop starts: `tasks` is a list of strings containing tasks; `running` is true; `completed` is the total count of 'complete' tasks in `tasks'; `pending` is the total count of 'pending' tasks in `tasks`.
```
for task in tasks:
    # the loop body is omit
```
