...
Your Task:
Initial State: *`tasks` is a list of strings with at least 1 task, `running` is True, `completed` is 0, `pending` is 0, `task` is the first task in the list, and the first task in the list is 'complete'.
```
completed += 1
