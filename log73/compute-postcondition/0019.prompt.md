...
Your Task:
Initial State: `tasks` is a list of strings with at least 3 tasks, `running` is True, `completed` is incremented by 1, `pending` remains 0, and `task` is the third task in the list
```
pending += 1
