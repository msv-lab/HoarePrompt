...
Your Task:
Precondition: `tasks` is a list of strings with at least 3 tasks, `running` is True, `completed` is incremented by 1, `pending` remains 0, and `task` is the third task in the list
Program Fragment:
```
if task == 'complete':
    completed += 1
...
```
if part: `tasks` is a list of strings with at least 3 tasks; `running` is True; `completed` is incremented by 1; `pending` remains 0; `task` is the third task in the list.
else part: *`tasks` is a list of strings with at least 3 tasks, `running` is True, `completed` is incremented by 1, and `pending` is either 1 if the third task is 'pending', or remains 0 if the third task is any other string, in which case 'Unknown task' is printed.
