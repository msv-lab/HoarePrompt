...
Your Task:
State at the end of the previous iteration: `counter` is now an integer greater than or equal to 0; `total` remains greater than its previous value by the amount of the original `counter`.
```
while counter > 0:
    # the loop body is omit
```
