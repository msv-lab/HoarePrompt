...
Output State after the loop executes 1 times: `counter` is an integer greater than 0, `total` is now greater than 0; `counter` is decreased by 1.
Output State after the loop executes 2 times: `counter` is now an integer greater than or equal to 0; `total` remains greater than its previous value by the amount of the original `counter`.
Output State after the loop executes 3 times: `counter` is now less than its previous value, `total` is greater than its previous value by the amount of the original `counter`.
