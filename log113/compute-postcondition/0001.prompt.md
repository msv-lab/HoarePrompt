...
Your Task:
Initial State: *`n` is a non-negative integer, `k` is a positive integer such that 1 <= `k` <= 10, and `dp` is a dictionary with keys as tuples of (`n`, `k`) and values as integers. The current value of `k` is 0.
```
return 0
