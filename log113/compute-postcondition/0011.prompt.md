...
Your Task:
Initial State: `n` is a non-negative integer greater than or equal to 10, `k` must be greater than or equal to 2, `dp` is a dictionary with keys as tuples of (`n`, `k`) and values as integers, `dp[n, k]` is now 9, `i` is 1
```
