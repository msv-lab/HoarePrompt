...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer greater than or equal to 10; `k` is greater than or equal to 2; `dp[n, k]` is updated to `90 + 9 * 10
```
for i in range(1, k):
    # the loop body is omit
```
