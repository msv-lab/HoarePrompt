...
Your Task:
Precondition: `numbers` is a list of integers with at least 2 integers, num is the second integer in the list
Program Fragment:
```
if num > 10:
    high += 1
...
```
if part: `numbers` is a list of integers with at least 2 integers, `num` is the second integer in the list, and `high` is increased by 1.
else part: *`numbers` is a list of integers containing at least 2 integers, and `num` is the second integer in this list. If `num` is greater than 0, then `medium` is increased by 1. Otherwise, if `num` is less than or equal to 0, `low` is increased by 1.
