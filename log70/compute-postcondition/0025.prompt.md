...
Your Task:
Initial State: `high` is the count of integers in `numbers` greater than 10, `medium` is the count of integers in `numbers` greater than 0 but less than or equal to 10, `low` is the count of integers in `numbers` less than or equal to 0, and `numbers` is a list of integers.
```
