...
Your Task:
Precondition: *`numbers` is a list of integers with at least 1 integer, `high` is 0, `medium` is 0, `low` is 0, `num` is the first integer in the `numbers` list, and `num` is less than or equal to 10.
Program Fragment:
```
if num > 0:
    medium += 1
...
```
if part: `numbers` is a list of integers with at least 1 integer, `high` is 0, `medium` is 1, `low` is 0, `num` is the first integer in the `numbers` list, `num` is less than or equal to 10, and `num` is greater than 0.
else part: `numbers` is a list of integers with at least 1 integer, `high` is 0, `medium` is 0, `low` is 1, `num` is the first integer in the `numbers` list, `num` is less than or equal to 10, and `num` is less than or equal to 0.
