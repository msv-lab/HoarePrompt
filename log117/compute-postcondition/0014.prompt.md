...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer less than 10^18, `k` is a positive integer greater than 2 and less than or equal to 10, `dp[n, k]` is now updated to `dp[n, k] + 9 * 10
```
for i in range(1, k):
    # the loop body is omit
```
