...
Your Task:
Initial State: *`numbers` is a list of integers with length greater than 5, `even_sum` is 0, `odd_sum` is 0, `num` is the first integer in the numbers list, and `num` is an even integer.
```
even_sum += num
