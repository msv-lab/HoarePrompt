...
Your Task:
State at the end of the previous iteration: *`numbers` is a list of integers with length greater than 6. If `num` is an even integer, then `even_sum` is increased by `num`. If `num` is an odd integer, `even_sum` remains unchanged.
```
for num in numbers:
    # the loop body is omit
```
