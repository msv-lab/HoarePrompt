...
Your Task:
Precondition: *`numbers` is a list of integers; `even_sum` is 0, `odd_sum` is 0, and the length of `numbers` is less than or equal to 5.
Program Fragment:
```
if len(numbers) > 0:
    for num in numbers:
...
```
if part: `numbers` is a list of integers with length between 1 and 5, `even_sum` is 0, `odd_sum` is equal to the sum of all integers in `numbers`.
else part: `numbers` is a list of integers, `even_sum` is 0, `odd_sum` is 0, the length of `numbers` is 0, and 'Empty list' is printed.
