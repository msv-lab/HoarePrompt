...
Your Task:
State before the loop starts: *`numbers` is a list of integers, `even_sum` is 0, `odd_sum` is 0, and the length of `numbers` is greater than 0 and less than or equal to 5.
```
for num in numbers:
    # the loop body is omit
```
