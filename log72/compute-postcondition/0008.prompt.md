...
Your Task:
Precondition: `numbers` is a list of integers with length greater than 5, `even_sum` is updated to the previous value plus `num` if `num` is an even integer; otherwise, `even_sum` remains unchanged, `odd_sum` remains 0, `num` is the second integer in the list.
Program Fragment:
```
if num % 2 == 0:
    even_sum += num
...
```
if part: `numbers` is a list of integers with length greater than 5; `even_sum` is updated to the previous value plus `num` if `num` is even; `odd_sum` remains 0.
else part: None
