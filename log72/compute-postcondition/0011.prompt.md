...
Your Task:
Precondition: `numbers` is a list of integers with length greater than 6, num is the next integer in the list
Program Fragment:
```
if num % 2 == 0:
    even_sum += num
...
```
if part: `numbers` is a list of integers with length greater than 6, `num` is the next integer in the list, `even_sum` is increased by `num
else part: None
