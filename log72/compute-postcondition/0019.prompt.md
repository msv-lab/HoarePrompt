...
Your Task:
Initial State: `numbers` is a list of integers with length between 2 and 5, `even_sum` is 0, `odd_sum` is equal to the second element in `numbers`, `num` is the second element in `numbers`
```
odd_sum += num
