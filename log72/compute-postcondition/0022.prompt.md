...
Your Task:
State at the end of the previous iteration: `numbers` is a list of integers with length at least 3, `even_sum` is 0, `odd_sum` is equal to `num, `num` is the third element in `numbers`
```
for num in numbers:
    # the loop body is omit
```
