...
Your Task:
State at the end of the previous iteration: *`numbers` is a list of integers with length greater than 5. If `num` is an even integer, then `even_sum` is updated to the previous value plus `num`, while `odd_sum` remains 0. If `num` is an odd integer, then `even_sum` remains unchanged, and `odd_sum` is still 0.
```
for num in numbers:
    # the loop body is omit
```
