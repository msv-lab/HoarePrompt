...
Your Task:
Precondition: `numbers` is a list of integers with length greater than 5, `even_sum` is 0, `odd_sum` is 0, `num` is the first integer in the numbers list
Program Fragment:
```
if num % 2 == 0:
    even_sum += num
...
```
if part: `numbers` is a list of integers with length greater than 5, `even_sum` is the previous value plus `num`, `odd_sum` is 0, `num` is an even integer
else part: None
