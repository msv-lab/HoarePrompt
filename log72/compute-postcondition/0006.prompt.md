...
Your Task:
State at the end of the previous iteration: *`numbers` is a list of integers with length greater than 5, `even_sum` is updated to the previous value plus `num` if `num` is an even integer; otherwise, `even_sum` remains unchanged. `odd_sum` remains 0. `num` is the first integer in the `numbers` list.
```
for num in numbers:
    # the loop body is omit
```
