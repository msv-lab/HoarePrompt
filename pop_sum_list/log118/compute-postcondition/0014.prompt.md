...
Initial State: `input_list` is a list of integers, `total_sum` is 0
Code of the loop:
```
while input_list:
...
Output State after the loop executes 1 times: *`input_list` contains at least one integer less than before, `total_sum` is equal to `element`, and `element` is a positive integer greater than 0.
Output State after the loop executes 2 times: *`input_list` has at least one integer remaining, `element` is a positive integer greater than 0, and `total_sum` is equal to the previous value plus `element`.
Output State after the loop executes 3 times: *`input_list` has at least one integer. If `element` is greater than 0, `total_sum` is updated to the previous value plus 2 times `element`.
