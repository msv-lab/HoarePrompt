...
Your Task:
Precondition: `input_list` has at least one integer remaining; `total_sum` is equal to `element`, and `element` is a positive integer greater than 0, now equal to the last integer popped from `input_list`.
Program Fragment:
```
if element > 0:

```
if part: `input_list` has at least one integer remaining; `total_sum` is equal to the previous value plus `element`, where `element` is a positive integer greater than 0.
else part: None
