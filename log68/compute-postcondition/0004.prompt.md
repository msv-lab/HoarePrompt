...
Your Task:
Precondition: `total` is 0, `numbers` is a list of integers with at least 1 integer, `num` is the first integer in the list
Program Fragment:
```
if num > 0:
    total += num
...
```
if part: `total` is now equal to `num`, `numbers` is a list of integers, `num` is the first integer in that list
else part: None
