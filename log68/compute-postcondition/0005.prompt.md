...
Your Task:
State at the end of the previous iteration: *`total` is 0, `numbers` is a list of integers, and `num` is the first integer in that list. If `num` is greater than 0, then `total` is equal to `num`. If `num` is less than or equal to 0, `total` remains 0.
```
for num in numbers:
    # the loop body is omit
```
