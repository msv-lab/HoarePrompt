...
Your Task:
Precondition: `total` is 0, `numbers` is a list of integers with at least 2 integers, `num` is the second integer in the list.
Program Fragment:
```
if num > 0:
    total += num
...
```
if part: `total` is `num`, `numbers` is a list of integers, `num` is the second integer in that list
else part: None
