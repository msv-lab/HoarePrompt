...
Your Task:
Precondition: `total` is 0, numbers contains at least 3 elements, num is the third integer in the list, index is 2
Program Fragment:
```
if num > 0:
    total += num
...
```
if part: `total` is `num`, where `num` is the third integer in the list; `index` is 2, and `numbers` contains at least 3 elements.
else part: None
