...
Your Task:
Precondition: *a, b, and c are integers. The value of a is greater than the value of b. The value of b is less than or equal to the value of c
Program Fragment:
```
if c > a:
    x = c - b
...
```
if part: a, b, and c are integers. The value of a is greater than the value of b. The value of b is less than or equal to the value of c. The value of c is greater than the value of a; x is a positive integer
else part: `a`, `b`, and `c` are integers. The value of `a` is greater than the value of `b`. The value of `b` is less than or equal to the value of `c`. The condition `(c > a)` is false. `x` is the sum of `a` and `b`
