...
Your Task:
State at the end of the previous iteration: `total` is the initial value of `total` plus the value of `num`, `numbers` is a list of integers with at least 1 integer, `num` is the first integer in the list
```
for num in numbers:
    # the loop body is omit
```
