...
```
if part: *`x` and `y` are integers, `x` is greater than 0. If `y` is greater than 0, `z` is the sum of `x` and `y`, which is also a positive integer. If `y` is less than or equal to 0, `z` is the difference between `x` and `y`, resulting in a positive integer.
else part: x is less than or equal to 0, y is an integer, z is an integer
