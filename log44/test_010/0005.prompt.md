...
Your Task:
Precondition: *x and y are integers, x is greater than 0
Program Fragment:
```
if y > 0:
    z = x + y
...
```
if part: `x` and `y` are integers, `x` is greater than 0, `y` is greater than 0, `z` is greater than 0
else part: `x` is greater than 0, `y` is less than or equal to 0, `z` is a positive integer
