...
Your Task:
Initial State: `matrix` is a 2D list of integers with at least 1 row, `total` is 0, `row` is the first row of the matrix with at least 1 element, `num` is the first element in the row
```
total += num
