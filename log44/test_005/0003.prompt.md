...
Your Task:
State at the end of the previous iteration: `matrix` is a 2D list of integers with at least 1 row, `total` is updated with the sum of the first element in the `row`, `row` is the first row of the matrix with at least 1 element, `num` is the first element in the `row`
```
for num in row:
    # the loop body is omit
```
