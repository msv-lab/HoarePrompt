...
Your Task:
State at the end of the previous iteration: `matrix` is a 2D list of integers with at least 1 row, `total` is the sum of all elements in the `row`, `row` is the last row of the matrix, `num` is the last element in the `row`
```
for row in matrix:
    # the loop body is omit
```
