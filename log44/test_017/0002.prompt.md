...
Initial State: numerator is a numeric value, denominator is a numeric value.
Code for the try except block:
```
try:
...
```
Output state after the execution of the try statement: numerator is a numeric value, denominator is a numeric value, result is the result of the division of numerator by denominator
Output state after the execution of the except statement(s): 
except_1: `result` is `None`
