...
Your Task:
Precondition: *`x` is an integer, `x` is less than or equal to 10, and `x` is less than or equal to 5.
Program Fragment:
```
if x > 0:

```
if part: `x` is an integer, `x` is less than or equal to 10, `x` is less than or equal to 5, `x` is greater than 0; `y` is either 100, 200, 300, 400, or 500 depending on the value of `x`.
else part: None
