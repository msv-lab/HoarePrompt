...
Initial State: `matrix` is a square 2D list of numbers with dimensions n x n; `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
Output State after the loop executes 1 times: `matrix` is a square 2D list of numbers with dimensions n x n, `total` is equal to `matrix[0][0] + matrix[0][n - 1]`, `i` is 0, `n` must be greater than 0
Output State after the loop executes 2 times: `matrix` is a square 2D list of numbers with dimensions n x n, `total` is equal to `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2], `i` is 1, `n` is greater than 1.
Output State after the loop executes 3 times: `matrix` is a square 2D list of numbers with dimensions n x n, `total` is equal to `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2] + matrix[2][2] + matrix[2][n - 3], `i` is 2, `n` is greater than 2.
