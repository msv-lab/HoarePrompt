...
Your Task:
Initial State: `matrix` is a square 2D list of numbers with dimensions n x n, `total` is equal to `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2], `i` is 2, `n` is greater than 2
```
total += matrix[i][i]
