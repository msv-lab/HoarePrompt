...
Your Task:
Initial State: `total` is equal to the sum of all elements along both the main diagonal and the anti-diagonal of `matrix`; `n` is a non-negative integer, `matrix` is a 2D list representing a square matrix of integers.
```
return total
