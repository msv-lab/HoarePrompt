...
Your Task:
Initial State: matrix is a 2D list representing a square matrix of integers, where the number of rows is equal to the number of columns (i.e., len(matrix) == len(matrix[0])).
```
n = len(matrix)
