...
Your Task:
State before the loop starts: `matrix` is a 2D list representing a square matrix of integers, `n` is the number of rows (and columns) in the matrix (i.e., `n` is equal to `len(matrix}`), `total` is 0
```
for i in range(n):
    # the loop body is omit
```
