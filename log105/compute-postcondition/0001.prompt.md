...
Your Task:
Initial State: `matrix` is a 2D list representing a square matrix of integers, `n` is the number of rows (and columns) in the matrix (i.e., `n` is equal to `len(matrix)`).
```
total = 0
