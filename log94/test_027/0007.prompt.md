...
```
if part: `x` is an integer, `y` is an integer, `z` is equal to `2 * x`
else part: `x` is an integer, `y` is an integer, `x` is not equal to `y`, `x` is less than or equal to `y`, `z` is a positive integer equal to `y - x`.
