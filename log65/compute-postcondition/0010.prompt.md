...
Your Task:
Precondition: *`x` is an integer, `x` is less than or equal to 0, and `x` is not equal to 0 (therefore, `x` is less than 0).
Program Fragment:
```
if x < -10:
    x -= 10
...
```
if part: `x` is less than or equal to 0, `x` is less than 0, `x` is less than -10; `x` is now less than -20
else part: `x` is less than or equal to 0, `x` is not equal to 0 (therefore, `x` is less than 0), and `x` is greater than or equal to -10; execution results in a contradiction regarding the value of `x`.
