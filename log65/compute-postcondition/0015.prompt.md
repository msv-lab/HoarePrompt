...
#Overall this is what the function does: The function accepts a parameter x, returns a value greater than 1 if x is greater than 0, returns 10 if x is 0, returns a value less than -20 if x is less than -10, and returns 0 if x is between -10 and 0.

```

Return Postconditions: The program returns a value for x which is either greater than 1 (if it started greater than 0), equal to 10 (if it started at 0), less than -20 (if it started less than -10), or exactly 0 (if it started between -10 and 0)
