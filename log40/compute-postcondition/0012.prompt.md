...
Your Task:
Initial State: `n` is a non-negative integer, `prev` is the last value of `curr` after the loop finishes, `curr` is the last value of `prev` after the loop finishes
```
return curr
