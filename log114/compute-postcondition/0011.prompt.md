...
Initial State: `matrix` is a 2D list of integers representing a square matrix of dimension n×n, `n` is the positive integer value equal to the length of `matrix`, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
