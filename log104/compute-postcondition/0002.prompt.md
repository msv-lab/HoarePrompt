...
Your Task:
State before the loop starts: `matrix` is a 2D list (square matrix) of integers, `n` is the length of one side of the square matrix (number of rows or columns), `total` is 0.
```
for i in range(n):
    # the loop body is omit
```
