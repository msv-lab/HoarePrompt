...
Initial State: `matrix` is a 2D list (square matrix) of integers, `n` is the length of one side of the square matrix (number of rows or columns), `total` is 0.
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
Output State after the loop executes 1 times: `total` is equal to `total + matrix[0][n - 1]`, `matrix` is a 2D list (square matrix) of integers, `n` must be greater than 0, `i` is still 0
Output State after the loop executes 2 times: `total` is equal to its previous value plus `matrix[1][1] + matrix[1][n - 2]`, `n` is greater than 1, `i` is 1
Output State after the loop executes 3 times: `total` is equal to its previous value plus `matrix[1][1] + matrix[1][n - 2] + matrix[2][2] + matrix[2][n - 3]`, `n` is greater than 2, `i` is 2
