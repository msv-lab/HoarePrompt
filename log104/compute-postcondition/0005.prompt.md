...
Your Task:
State at the end of the previous iteration: `total` is equal to `total + matrix[0][n - 1]`, `matrix` is a 2D list (square matrix) of integers, `n` must be greater than 0, `i` is still 0
```
for i in range(n):
    # the loop body is omit
```
