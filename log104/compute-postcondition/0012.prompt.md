...
Your Task:
Initial State: `total` is equal to the sum of all elements along the primary and secondary diagonals of `matrix`, `matrix` is a 2D list (square matrix) of integers, `n` is a positive integer indicating the size of the matrix.
```
return total
