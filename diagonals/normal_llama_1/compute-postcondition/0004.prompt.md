...
Your Task:
Initial State: `matrix` is a square list of lists of integers with at least 1 row (or column), `n` is at least 1, `total` is the integer at the first row and first column of the `matrix`, `i` is 0
```
