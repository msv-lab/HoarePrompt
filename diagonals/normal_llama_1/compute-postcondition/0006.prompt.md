...
Your Task:
Initial State: `matrix` is a square list of lists of integers with at least 2 rows (or columns), `n` is at least 2, `total` is the sum of the integers at the first row and first column and the first row and last column of the `matrix`, `i` is 1
```
total += matrix[i][i]
