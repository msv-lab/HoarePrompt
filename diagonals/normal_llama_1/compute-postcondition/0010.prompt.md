...
Your Task:
Initial State: `matrix` is a square list of lists of integers with at least 2 rows (or columns), `n` is at least 2, `total` is the old value of `total` plus the integer at the 2nd row and 2nd column of the `matrix`, `i` is 2
```
