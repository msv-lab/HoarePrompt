...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is the updated value after adding matrix[0][n - 1], `i` is 0, `n` is greater than 0
```
for i in range(n):
    # the loop body is omit
```
