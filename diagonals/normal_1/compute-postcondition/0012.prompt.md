...
Initial State: matrix is a square matrix (n x n) where n is a positive integer; `n` is the length of the matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is the updated value after adding matrix[0][n - 1], `i` is 0, `n` is greater than 0
Output State after the loop executes 2 times: `matrix` is a square matrix (n x n), `total` is the updated value after adding matrix[0][n - 1] and matrix[1][n - 2], `i` is 1, `n` is greater than 0
Output State after the loop executes 3 times: `matrix` is a square matrix (n x n), `total` is the updated value after adding matrix[0][n - 1], matrix[1][n - 2], and matrix[2][n - 3] + matrix[2][2]
