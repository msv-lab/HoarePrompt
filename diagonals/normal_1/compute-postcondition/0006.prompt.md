...
Your Task:
Initial State: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is the updated value after adding matrix[0][n - 1], `i` is 1, `n` is greater than 0
```
total += matrix[i][i]
