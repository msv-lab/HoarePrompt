...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix (n x n), `total` is the updated value after adding matrix[0][n - 1] and matrix[1][n - 2], `i` is 1, `n` is greater than 0
```
for i in range(n):
    # the loop body is omit
```
