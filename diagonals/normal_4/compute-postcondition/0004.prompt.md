...
Your Task:
Initial State: matrix is a square matrix (n x n) where n is a positive integer, total is updated with the value at matrix[0][0], `i` is 0, `n` is a positive integer
```
