...
Your Task:
Initial State: matrix is a square matrix (n x n) where n is a positive integer, total is updated with the sum of the values at matrix[0][0] and matrix[0][n-1], `i` is 1, `n` is a positive integer
```
total += matrix[i][i]
