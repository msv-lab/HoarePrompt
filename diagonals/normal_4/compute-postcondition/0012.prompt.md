...
Initial State: matrix is a square matrix (n x n) where n is a positive integer, total is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: matrix is a square matrix (n x n) where n is a positive integer, total is updated with the sum of the values at matrix[0][0] and matrix[0][n-1], `i` is 0, `n` is a positive integer
Output State after the loop executes 2 times: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is updated with the sum of the values at `matrix[0][0]` and `matrix[0][n-1]` and `matrix[1][1]`, `i` is 1, `n` is a positive integer
Output State after the loop executes 3 times: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is updated with the sum of the values at `matrix[0][0]`, `matrix[0][n-1]`, `matrix[1][1]`, `matrix[i][i]`, `i` is 1, `n` is a positive integer, `total` is increased by the value at `matrix[i][n - i - 1]`
