...
Your Task:
State at the end of the previous iteration: `matrix` is a square list of lists, `n` is greater than 1, `i` is 1, and `total` is `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2]`.
```
for i in range(n):
    # the loop body is omit
```
