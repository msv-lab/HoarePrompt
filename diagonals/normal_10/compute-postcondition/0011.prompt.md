...
Your Task:
State at the end of the previous iteration: `n` is greater than 0, `total` is the updated value after adding `matrix[1][n - 1]`, `matrix[1][1]`, `matrix[2][2]`, and `matrix[2][n - 3]`, `i` is 2
```
for i in range(n):
    # the loop body is omit
```
