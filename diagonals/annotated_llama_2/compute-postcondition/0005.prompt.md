...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers with at least 1 inner list, `n` is greater than 0, `i` is 0, `total` is `matrix[0][0] + matrix[0][n - 1]`.
```
for i in range(n):
    # the loop body is omit
```
