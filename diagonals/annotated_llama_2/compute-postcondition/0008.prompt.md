...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers with at least 1 inner list, `n` is greater than 1, `i` is 1, and `total` is `matrix[1][0] + matrix[1][n - 1] + matrix[1][1] + matrix[1][n - 2]`.
```
for i in range(n):
    # the loop body is omit
```
