...
Your Task:
Initial State: `matrix` is a square 2D list of integers with at least 1 inner list, `n` is greater than 2, `i` is 2, and `total` is some value based on `matrix[2][0]`, `matrix[2][n - 1]`, `matrix[2][1]`, and `matrix[2][n - 2]`.
```
total += matrix[i][i]
