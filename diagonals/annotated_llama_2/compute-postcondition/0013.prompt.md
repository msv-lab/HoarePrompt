...
Your Task:
Initial State: If `n` is greater than 0, `total` is the sum of all elements on the main diagonal and the anti-diagonal of the matrix, `i` is `n - 1`, and `matrix` is a square 2D list of integers. If `n` is 0, the loop doesn't execute, and `total` remains 0.
```
return total
