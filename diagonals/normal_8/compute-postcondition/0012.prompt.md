...
Initial State: matrix remains a square matrix (n x n) where n is a positive integer greater than 0, and each element of the matrix is a numerical value
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: matrix remains a square matrix (n x n) where n is a positive integer greater than 0, and each element of the matrix is a numerical value; `total` is increased by the sum of diagonal elements of the matrix
Output State after the loop executes 2 times: Example Answer:
...
Output State after the loop executes 3 times: total is increased by the sum of the diagonal elements of the new matrix; `i` is incremented by 1
