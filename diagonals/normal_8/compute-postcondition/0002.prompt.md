...
Your Task:
State before the loop starts: matrix remains a square matrix (n x n) where n is a positive integer greater than 0, and each element of the matrix is a numerical value
```
for i in range(n):
    # the loop body is omit
```
