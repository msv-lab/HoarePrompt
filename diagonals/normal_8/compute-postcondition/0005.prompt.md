...
Your Task:
State at the end of the previous iteration: matrix remains a square matrix (n x n) where n is a positive integer greater than 0, and each element of the matrix is a numerical value; `total` is increased by the sum of diagonal elements of the matrix
```
for i in range(n):
    # the loop body is omit
```
