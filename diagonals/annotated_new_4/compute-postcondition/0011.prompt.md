...
Your Task:
State at the end of the previous iteration: `total` is the updated value after adding the new element from the matrix, `i` is incremented by 1, `n` remains unchanged
```
for i in range(n):
    # the loop body is omit
```
