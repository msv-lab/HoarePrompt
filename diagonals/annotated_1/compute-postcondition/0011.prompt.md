...
Your Task:
State at the end of the previous iteration: `total` is updated with the sum of the previous total and the value of the element in the 2nd row and last column of the matrix, `i` is incremented by 1, `n` is greater than or equal to `i+1
```
for i in range(n):
    # the loop body is omit
```
