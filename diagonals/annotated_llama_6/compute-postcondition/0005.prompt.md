...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers with at least 1 row and column, `n` is at least 1, `total` is `matrix[0][0] + matrix[0][n - 1]`, `i` is 0
```
for i in range(n):
    # the loop body is omit
```
