...
#Overall this is what the function does: The function accepts a square 2D list of integers as a parameter and returns the sum of the elements on the main diagonal and the anti-diagonal of the matrix.

```

Return Postconditions: `n` is either 0 or greater than 0, `matrix` is a square 2D list of integers with `n` rows and columns, `total` is either 0 or the sum of the elements on the main diagonal and the anti-diagonal of the matrix, and the function returns `total`.
