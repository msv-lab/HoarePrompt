...
Your Task:
Initial State: `matrix` is a square matrix (n x n), `n` is greater than 0, `total` is the updated sum of the diagonal elements of the matrix + matrix[n][n], `i` is equal to `n`
```
