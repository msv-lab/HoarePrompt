...
Your Task:
Initial State: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is the value at the diagonal position of the first row and column, `i` is 0
```
