...
```
Output State after the loop executes 1 times: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is updated based on the diagonal element, `i` is 0
Output State after the loop executes 2 times: matrix is a square matrix (n x n) where n is a positive integer, total is updated based on the diagonal element, i is 1
Output State after the loop executes 3 times: `matrix` is a square matrix (n x n) where n is a positive integer, `total` is updated based on the diagonal element, `i` is 2
