...
Your Task:
State at the end of the previous iteration: `total` is updated with the new sum of values in the diagonal from the top right to the bottom left of the matrix, `i` is incremented by 1
```
for i in range(n):
    # the loop body is omit
```
