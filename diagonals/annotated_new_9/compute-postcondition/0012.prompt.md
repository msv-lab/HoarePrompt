...
Initial State: matrix is a square matrix (n x n) where n is a positive integer, `n` is the number of rows or columns in the matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: total is updated with the new diagonal element
Output State after the loop executes 2 times: `total` is updated with the new diagonal element value, `i` is incremented by 1, `n` remains the same
Output State after the loop executes 3 times: `total` is updated with the new diagonal element value, `i` is less than `n+1`, `n` remains the same
