...
Your Task:
State at the end of the previous iteration: `matrix` is a square list of lists of integers with at least 1 row and column, `n` is greater than 0, `total` is the initial total (0) plus the value at `matrix[0][0]` plus the value at `matrix[0][n - 1]`, `i` is 0
```
for i in range(n):
    # the loop body is omit
```
