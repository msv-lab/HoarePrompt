...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix of size n x n where n is a positive integer, `total` is the updated sum of all diagonal elements in the matrix
```
for i in range(n):
    # the loop body is omit
```
