...
Your Task:
Initial State: `matrix` is a 2D list representing a square matrix, `n` is greater than 1, `i` is 1, `total` is the initial value of `total` plus the value of `matrix[0][0]` and `matrix[0][n - 1]`.
```
total += matrix[i][i]
