...
Your Task:
Initial State: matrix is a 2D list representing a square matrix, i.e., a list of lists where each inner list has the same length as the outer list, and all elements in the matrix are numbers.
```
n = len(matrix)
