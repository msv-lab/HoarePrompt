...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix (n x n) where n is a non-negative integer, `n` is greater than 0, `total` is the sum of diagonal elements in the matrix
```
for i in range(n):
    # the loop body is omit
```
