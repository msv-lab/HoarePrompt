...
Your Task:
State at the end of the previous iteration: `total` is updated according to the new value at position [1][1] in the matrix, `i` is incremented by 1, `n` remains the same
```
for i in range(n):
    # the loop body is omit
```
