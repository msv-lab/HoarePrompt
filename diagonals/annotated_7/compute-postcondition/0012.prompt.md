...
Initial State: `matrix` is a square 2D list of integers of dimension n x n, where n is a non-negative integer; `n` is the dimension of the square matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: `matrix` is a square 2D list of integers of dimension n x n, `n` is a non-negative integer, `n` is greater than or equal to 1, `total` is the sum of the diagonal elements up to index 0, `i` is 0
Output State after the loop executes 2 times: `matrix` is a square 2D list of integers of dimension n x n, `n` is a non-negative integer, `total` is the sum of the diagonal elements up to index 0 plus `matrix[i][i]` plus `matrix[i][n - i - 1]`, `i` is 1
Output State after the loop executes 3 times: `matrix` is a square 2D list of integers of dimension n x n, `n` is a non-negative integer, `total` is updated according to the mentioned formula
