...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers with dimensions n x n, `n` is the side length of the square matrix, `total` is the updated sum of the diagonal elements of the matrix up to the new ith element, `i` is increased by 1, `n` is greater than 0
```
for i in range(n):
    # the loop body is omit
```
