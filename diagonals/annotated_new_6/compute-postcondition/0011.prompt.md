...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix (n x n) where n is a non-negative integer, `total` is updated to include the diagonal elements from (0, 0) to (n-1, n-1), `i` is increased by 1
```
for i in range(n):
    # the loop body is omit
```
