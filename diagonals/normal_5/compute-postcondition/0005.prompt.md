...
Your Task:
State at the end of the previous iteration: `total` is updated based on the sum of the initial value and the element at the opposite end of the row, `i` is 0, `n` is greater than 0
```
for i in range(n):
    # the loop body is omit
```
