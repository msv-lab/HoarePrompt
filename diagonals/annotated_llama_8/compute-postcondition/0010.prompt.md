...
Your Task:
Initial State: `matrix` is a 2D list of integers, `n` is greater than 2, `total` is the sum of the top-left, top-right, middle elements of the first row of `matrix`, the second element of the second row of `matrix`, and `matrix[2][2]`, `i` is 2
```
