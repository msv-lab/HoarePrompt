...
Your Task:
Initial State: If `n` is greater than 0, `total` is the sum of the elements on the main diagonal and the anti-diagonal of the square matrix, and `i` is `n-1`. If `n` is 0, the loop does not execute, and `total` is 0. `matrix` is a 2D list of integers.
```
return total
