...
Your Task:
Initial State: `matrix` is a 2D list of integers, `n` is greater than 2, `total` is the sum of the top-left, top-right, middle elements of the first row of `matrix` and the second element of the second row of `matrix`, `i` is 2
```
total += matrix[i][i]
