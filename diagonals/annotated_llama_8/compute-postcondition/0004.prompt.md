...
Your Task:
Initial State: `matrix` is a 2D list of integers representing a square matrix with at least 1 row (and column), `n` is greater than 0, `total` is the value of the top-left element of `matrix`, `i` is 0
```
