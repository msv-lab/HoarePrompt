...
Your Task:
State at the end of the previous iteration: `matrix` is a square list of lists of integers, `n` is at least 1, `total` is the sum of the first element of the first sublist and the nth element of the first sublist in `matrix`, `i` is 0
```
for i in range(n):
    # the loop body is omit
```
