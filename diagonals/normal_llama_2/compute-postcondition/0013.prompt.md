...
Your Task:
Initial State: If `n` is at least 1, then `total` is the sum of the elements on the main diagonal and the anti-diagonal of the matrix, `i` is `n-1`, and `matrix` is a square list of lists of integers. If `n` is 0, the loop does not execute, and `total` is 0.
```
return total
