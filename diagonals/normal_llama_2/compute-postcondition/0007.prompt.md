...
Your Task:
Initial State: `matrix` is a square list of lists of integers, `n` is at least 2, `total` is the sum of the first element of the first sublist, the nth element of the first sublist, and the element at the second row and second column in `matrix`, `i` is 1
```
