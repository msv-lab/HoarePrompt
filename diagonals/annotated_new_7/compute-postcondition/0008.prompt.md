...
Your Task:
State at the end of the previous iteration: `total` is updated based on the value at the diagonal of the matrix, `i` is incremented by 1, `n` is greater than the new value of `i`
```
for i in range(n):
    # the loop body is omit
```
