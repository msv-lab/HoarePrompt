...
def func_1(matrix):
    n = len(matrix)
    #State of the program here: matrix is a square 2D list of integers with dimensions n x n, where n is a non-negative integer
    total = 0
    ...
    #State of the program after the  for loop has been executed: total is updated based on the values at the diagonals of the matrix. If n is less than or equal to 0, the loop does not execute and total remains 0
    return total
    #State of the program after the return statement: `total` is updated based on the values at the diagonals of the matrix. If `n` is less than or equal to 0, the loop does not execute and `total` remains 0
#Overall this is what the function does: The function accepts a square 2D list of integers matrix with dimensions n x n. It updates the variable total based on the values at the diagonals of the matrix. If n is less than or equal to 0, total remains 0.

```

Return Postconditions: `total` is updated based on the values at the diagonals of the matrix. If `n` is less than or equal to 0, the loop does not execute and `total` remains 0
