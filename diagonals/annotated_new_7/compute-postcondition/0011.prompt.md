...
Your Task:
State at the end of the previous iteration: `total` is updated based on the value at the diagonal of the matrix, `i` is incremented by 2, `n` is greater than the new value of `i + 1`
```
for i in range(n):
    # the loop body is omit
```
