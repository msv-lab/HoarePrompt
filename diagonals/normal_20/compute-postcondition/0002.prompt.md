...
Your Task:
State before the loop starts: matrix is a square matrix (n x n) where n is a positive integer greater than 0; `n` represents the size of the square matrix, `total` is 0
```
for i in range(n):
    # the loop body is omit
```
