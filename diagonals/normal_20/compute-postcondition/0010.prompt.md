...
Your Task:
Initial State: `matrix` is a square matrix ((n+1) x (n+1)), `total` is the updated sum of all diagonal elements of the matrix after adding matrix[i][i], and `i` is less than n+1
```
