...
Your Task:
Initial State: `matrix` is a square matrix of integers with at least 1 row (and column), `n` is greater than 0, `total` is the value at the position `[0][0]` in the `matrix`, `i` is 0
```
