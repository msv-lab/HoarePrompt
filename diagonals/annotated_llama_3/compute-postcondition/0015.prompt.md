...
def func_1(matrix):
    n = len(matrix)
    #State of the program here: `matrix` is a square matrix of integers, `n` is the number of rows (or columns) of the `matrix`
    total = 0
    #State of the program here: `matrix` is a square matrix of integers, `n` is the number of rows (or columns) of the `matrix`, `total` is 0
    for i in range(n):
        total += matrix[i][i]
    ...
    return total
    #State of the program after the return statement: the function returns `total`
#Overall this is what the function does: The function accepts a square matrix of integers and returns a total value.

```

Return Postconditions: the function returns `total`
