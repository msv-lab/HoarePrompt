...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix of integers with dimensions n x n where n is at least 1, and `total` is the updated sum of all diagonal elements of the matrix
```
for i in range(n):
    # the loop body is omit
```
