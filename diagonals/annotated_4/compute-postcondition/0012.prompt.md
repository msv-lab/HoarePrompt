...
Initial State: matrix is a square matrix (n x n) where n is a non-negative integer, `n` is the side length of the square matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: matrix is a square matrix of side length `n`, `total` is the updated value after adding the value at position `(0, n-1)` to it, `n` is greater than 0, `i` is 0
Output State after the loop executes 2 times: `matrix` is a square matrix of side length `n`, `total` is the updated value after adding the value at position `(0, n-1)` and `matrix[i][i]`, `n` is greater than 0, `i` is 1
Output State after the loop executes 3 times: `matrix` is a square matrix of side length `n`, `total` is the updated value after adding the value at position `(0, n-1)` and `matrix[i][i]`, `n` is greater than 0, `i` is 2, `total` is increased by the value at `matrix[2][2]`
