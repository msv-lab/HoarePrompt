...
Your Task:
State at the end of the previous iteration: matrix is a square matrix with dimensions n x n, `total` is the sum of the diagonal and anti-diagonal elements of the matrix, `n` is greater than 0, `i` is n
```
for i in range(n):
    # the loop body is omit
```
