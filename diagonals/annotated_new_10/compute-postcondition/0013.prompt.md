...
Your Task:
Initial State: `total` is the sum of the diagonal and anti-diagonal elements of the matrix, `n` is a non-negative integer, `i` is at least 0. If `n` is 0, the loop does not execute and `total` remains 0.
```
return total
