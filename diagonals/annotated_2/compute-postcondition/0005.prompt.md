...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix of dimension n x n where n is a non-negative integer and n is greater than 0, `total` is the sum of the diagonal elements up to the (i+1)th index, `i` is increased by 1
```
for i in range(n):
    # the loop body is omit
```
