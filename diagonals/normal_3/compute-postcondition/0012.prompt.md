...
Initial State: `n` is the length of the matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: `n` is greater than 0, `total` is the updated value after adding `matrix[i][n - i - 1]`, `i` is 0
Output State after the loop executes 2 times: `n` is greater than 0, `total` is the updated value after adding `matrix[1][n - 1]` and `matrix[1][1]`, `i` is 1
Output State after the loop executes 3 times: `n` is greater than 0, `total` is the updated value after adding `matrix[1][n - 1]`, `matrix[1][1]`, `matrix[2][2]`, and `matrix[2][n - 3]`, `i` is 2
