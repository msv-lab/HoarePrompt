...
Your Task:
Precondition: `numbers` is a list of integers, `i` is 1, `skipped` is either 0 or 1, and the length of `numbers` is greater than 1
Program Fragment:
```
if numbers[i] > 0:
    total += numbers[i]
...
```
if part: `numbers` is a list of integers, `i` is 1, `skipped` is either 0 or 1; `total` is updated by adding `numbers[1]`.
else part: `numbers` is a list of integers, `i` is 1, `skipped` is either 1 or 2
