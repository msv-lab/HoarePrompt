...
Your Task:
State at the end of the previous iteration: `numbers` is a list of integers with length greater than 2; `total` is unchanged; `skipped` is either 1 or 2; `i` is increased by 1.
```
while i < len(numbers):
    # the loop body is omit
```
