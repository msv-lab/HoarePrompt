...
Your Task:
State at the end of the previous iteration: `numbers` is a list of integers, `i` is 1, `skipped` is either 0 or 1, and `total` is either the value of `numbers[0]` (if `numbers[0]` is greater than 0) or remains 0 (if `numbers[0]` is less than or equal to 0)
```
while i < len(numbers):
    # the loop body is omit
```
