...
Your Task:
Precondition: `numbers` is a list of integers, `i` is 2, `skipped` is either 0 or 1, and len(numbers) is greater than 2
Program Fragment:
```
if numbers[i] > 0:
    total += numbers[i]
...
```
if part: `numbers` is a list of integers, `i` is 2, `skipped` is either 0 or 1, and `total` is increased by `numbers[2]
else part: `numbers` is a list of integers, `i` is 2, `skipped` is either 1 or 2, and `len(numbers)` is greater than 2.
