...
Initial State: counter is an integer greater than 0, total is 0
Code of the loop:
```
while counter > 0:
    total += counter
...
Output State after the loop executes 1 times: `counter` is an integer greater than 0; `total` is now equal to the previous value of `counter`; `counter` is decreased by 1
Output State after the loop executes 2 times: `counter` is 1 or greater, `total` is now equal to 2 times the previous value of `counter`, and `counter` is decreased by 1.
Output State after the loop executes 3 times: `counter` is now less than or equal to the previous value, and `total` remains equal to 2 times the previous value of `counter` plus the current value of `counter`.
