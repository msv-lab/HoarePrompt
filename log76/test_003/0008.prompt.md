...
Your Task:
State at the end of the previous iteration: `counter` is now less than or equal to the previous value, and `total` remains equal to 2 times the previous value of `counter` plus the current value of `counter`.
```
while counter > 0:
    # the loop body is omit
```
