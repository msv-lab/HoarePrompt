...
Your Task:
State at the end of the previous iteration: `counter` is an integer greater than 0; `total` is now equal to the previous value of `counter`; `counter` is decreased by 1
```
while counter > 0:
    # the loop body is omit
```
