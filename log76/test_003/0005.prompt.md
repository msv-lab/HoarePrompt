...
Your Task:
State at the end of the previous iteration: `counter` is 1 or greater, `total` is now equal to 2 times the previous value of `counter`, and `counter` is decreased by 1.
```
while counter > 0:
    # the loop body is omit
```
