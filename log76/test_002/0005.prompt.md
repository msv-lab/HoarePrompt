...
Your Task:
Initial State: `total` is the original value plus `num`, for the loop to execute a third time `numbers` is a list of integers with at least 3 integers, `num` is the third integer in the list
```
total += num
