...
Your Task:
Precondition: `numbers` is a list of integers with at least 1 more element, `total` is (previous total value), `total_product` is (previous total_product value), `num` is the next integer in the list
Program Fragment:
```
if num != 0:
    total *= num
...
```
if part: `numbers` is a list of integers with at least 1 more element, `total` is (previous total value multiplied by `num`), `total_product` is (previous total_product value)
else part: `numbers` is a list of integers with at least 1 more element, `total` is 1, `total_product` is (previous total_product value + previous total), `num` is the next integer in the list.
