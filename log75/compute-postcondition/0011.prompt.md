...
Your Task:
Precondition: `numbers` is a list of integers with at least 1 element, `total` is 1, `total_product` is 0, `num` is the first integer in `numbers`
Program Fragment:
```
if num != 0:
    total *= num
...
```
if part: `numbers` is a list of integers, `total` is now equal to `num`, `total_product` is 0, and `num` is the first integer in `numbers`
else part: `numbers` is a list of integers with at least 1 element, `total` is 1, `total_product` is 1, `num` is the first integer in `numbers`, and `num` is equal to 0.
