...
Your Task:
State before the loop starts: *`numbers` is a list of integers where the integers can be zero or non-zero; `total` is 1; `total_product` is 0; the length of `numbers` is greater than 0.
```
for num in numbers:
    # the loop body is omit
```
