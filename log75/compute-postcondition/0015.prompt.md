...
Your Task:
Initial State: `numbers` is a list of integers with at least 1 more element, `total` is (previous total value), `total_product` is (previous total_product value + previous total), `num` is the next integer in the list
```
total = 1
