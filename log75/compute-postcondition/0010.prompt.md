...
Your Task:
Initial State: `numbers` is a list of integers with at least 1 element, `total` is 1, `total_product` is 1, `num` is the first integer in `numbers`, and `num` is equal to 0.
```
total = 1
