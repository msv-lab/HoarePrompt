...
Your Task:
Precondition: `numbers` is a list of integers with at least two elements, num is the next element in the list
Program Fragment:
```
if num != 0:
    total *= num
...
```
if part: `total` is multiplied by `num`, where `num` is the next element in the list `numbers`
else part: `numbers` is a list of integers with at least two elements, `num` is the next element in the list, `total_product` is increased by `total`, and `total` is 1.
