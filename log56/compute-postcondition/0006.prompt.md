...
#Overall this is what the function does: The function accepts a parameter x, returns x + 1 if x is greater than 0, and returns -1 if x is equal to 0.

```

Return Postconditions: Case_1: The program returns a non-negative integer greater than 1, specifically x + 1 where x is greater than 0

Case_2: The program returns the non-negative integer x, which is equal to -1
