...
Your Task:
Initial State: `total` is equal to `matrix[0][0] + matrix[0][n - 1]`, `matrix` is a 2D list representing a square matrix of dimensions n x n where n must be greater than 1, `i` is 1
```
total += matrix[i][i]
