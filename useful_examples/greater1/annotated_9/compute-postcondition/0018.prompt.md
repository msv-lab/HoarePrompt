...
Your Task:
Precondition: `numbers` has one less element, `total` and `count` may have been updated, and a message may have been printed
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` has one less element, `total` is updated, `count` may have been updated, and a message may have been printed
else part: `numbers` has one less element, `total` and `count` may have been updated, and a message 'Skipping non-positive number:', followed by the value of `num` has been printed
