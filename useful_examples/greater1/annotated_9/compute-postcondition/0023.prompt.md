...
```
if part: `numbers` is empty, `total` is the sum of all numbers greater than 1 that were removed, `count` is the count of numbers greater than 1, and there are no non-positive numbers left in the list
else part: `numbers` is empty, `total` contains the sum of all numbers greater than 1 that were removed, `count` represents the count of numbers greater than 1, `average` is calculated as `total / count`
