...
Your Task:
Precondition: `numbers` is a list of integers with one less element, `total` is 0, `count` is 0, `num` is the integer popped from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with one less element, `total` is the sum of all popped integers, `count` is 1, `num` is the integer popped from the list
else part: `numbers` is a list of integers with one less element, `total` is 0, `count` is 0
