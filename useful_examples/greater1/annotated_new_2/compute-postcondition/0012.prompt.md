...
Your Task:
Precondition: `numbers` has at least one element left, `total` and `count` may or may not be updated based on the last element `num`
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` has at least one element left, `total` is updated based on the last element `num`, `count` is incremented by 1
else part: `numbers` has at least one element left, `total` and `count` may or may not be updated based on the last element `num`
