...
Your Task:
State at the end of the previous iteration: *`numbers` has one less element, `num` holds the removed element. If `num` is greater than 1, the total is increased by `num`, and the count is increased by 1. If `num` is not greater than 1, a message is printed stating that a non-positive number is being skipped.
```
while numbers:
    # the loop body is omit
```
