...
Your Task:
Precondition: `numbers` is a list of integers with at least one element left; `num` is the popped integer value
Program Fragment:
```
if num > 1:
    total += num
...
else part: `numbers` is a list of integers with at least one element left, `num` is the popped integer value
