...
Your Task:
Initial State: `numbers` is a list of integers with the last element removed, `total` is 0, `count` is 0, and `num` is the last element that was removed from the list
```
total += num
