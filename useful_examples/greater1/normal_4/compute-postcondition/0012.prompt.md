...
Your Task:
Precondition: `numbers` list has one less element, `total` and `count` adjusted based on the last removed `num` from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` list has one less element, `total` is adjusted based on the last removed `num` from the list, `count` is increased by 1
else part: `numbers` list has one less element, `total` and `count` adjusted based on the last removed `num` from the list
