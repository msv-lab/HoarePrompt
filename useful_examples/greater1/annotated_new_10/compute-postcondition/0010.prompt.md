...
Your Task:
Initial State: `numbers` is a list of numbers, `total` is increased by the last element removed from `numbers` if greater than 1, `count` is increased by 1 if the last element removed from `numbers` was greater than 1.
```
count += 1
