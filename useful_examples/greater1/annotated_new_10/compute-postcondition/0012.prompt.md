...
Your Task:
Precondition: If the last element removed from `numbers` was greater than 1, then `total` is increased by that value and `count` is increased by 1. If the last element removed was not greater than 1, then a non-positive number was skipped.
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of numbers, `total` is increased by the last element removed from `numbers` if greater than 1, `count` is increased by 1
else part: If the last element removed from `numbers` was greater than 1, then `total` is increased by that value and `count` is increased by 1. If the last element removed was not greater than 1, then a non-positive number was skipped.
