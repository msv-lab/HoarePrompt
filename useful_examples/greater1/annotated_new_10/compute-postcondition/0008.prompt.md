...
Your Task:
Initial State: If the last element removed from `numbers` was greater than 1, then `total` should be increased by that value and `count` should be increased by 1. If the last element removed was not greater than 1, then a non-positive number was skipped.
```
num = numbers.pop()
