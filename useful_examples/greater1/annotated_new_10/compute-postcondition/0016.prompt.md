...
Your Task:
Initial State: `numbers` is a list of numbers with the last element removed; `num` is the removed last element, `total` is the sum of the previous total and the removed last element
```
count += 1
