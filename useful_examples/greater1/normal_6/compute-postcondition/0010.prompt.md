...
Your Task:
Initial State: `numbers` has one less element, `total` is the sum of all elements in `numbers` that are greater than 1 plus the value of `num`, `count` is the number of elements in `numbers` that are greater than 1, and `num` is the last popped integer
```
count += 1
