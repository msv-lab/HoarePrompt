...
Your Task:
Precondition: `numbers` list is updated based on the pop operation, `num` holds the value popped from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` list is updated based on the pop operation, `num` holds the value popped from the list, `total` is updated with the value of `num`, `count` is incremented by 1
else part: `numbers` list is updated based on the pop operation, `num` holds the value popped from the list
