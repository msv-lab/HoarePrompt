...
Your Task:
Initial State: `numbers` still has elements in it, `num` is assigned a new value from the list, `total` and `count` may be adjusted based on the new value of `num`.
```
num = numbers.pop()
