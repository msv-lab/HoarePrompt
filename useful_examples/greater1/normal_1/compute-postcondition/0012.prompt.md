...
Your Task:
Precondition: `numbers` is a list of integers with at least one element removed, `num` is the popped integer
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with at least one element removed; `num` is the popped integer; `total` is increased by the value of `num`; `count` is incremented by 1
else part: `numbers` is a list of integers with at least one element removed, `num` is the popped integer
