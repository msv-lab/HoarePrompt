...
Your Task:
Precondition: `total` and `count` are integers, `numbers` is a non-empty list with one less element
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `total` is updated according to the sum of the previous total and `num`, `count` is incremented by 1, and `numbers` is a non-empty list with one less element
else part: `total` and `count` are integers, `numbers` is a non-empty list with one less element
