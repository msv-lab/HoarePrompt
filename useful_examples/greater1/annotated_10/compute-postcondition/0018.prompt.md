...
Your Task:
Precondition: `total`, `count` are integers, `numbers` is a non-empty list with one less element, `num` is assigned the last element of the original list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `total` is updated with the addition of `num`, `count` is increased by 1, `numbers` still has one less element, and `num` is the last element of the original list
else part: `total`, `count` are integers, `numbers` is a non-empty list with one less element, `num` is assigned the last element of the original list
