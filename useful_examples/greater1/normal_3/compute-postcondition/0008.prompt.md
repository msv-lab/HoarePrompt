...
Your Task:
Initial State: `numbers` is a list of integers with at least one element, `total` and `count` are updated based on the value of `num` as mentioned in the initial state statement. If `num` is not positive, a message is printed as described.
```
num = numbers.pop()
