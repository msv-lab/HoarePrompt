...
Your Task:
Precondition: `numbers` is a list of integers with at least one element (one less element after `pop()`), `total` and `count` are updated based on the value of `num` as mentioned in the initial state statement. If `num` is not positive, a message is printed.
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with at least one element (one less element after `pop()`), `total` and `count` are updated based on the value of `num` as mentioned in the initial state statement. If `num` is not positive, a message is printed. The count is increased by 1
else part: `numbers` is a list of integers with one less element (after `pop()`), `total` and `count` are updated based on the value of `num` as mentioned in the initial state statement. If `num` is not positive, a message is printed.
