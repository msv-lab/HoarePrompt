...
Your Task:
Initial State: `numbers` is a list of integers with one less element, `num` is the popped integer from the list. If the popped integer is less than or equal to 1, `total` is 0, `count` is 0.
```
