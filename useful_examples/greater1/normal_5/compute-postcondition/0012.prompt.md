...
Your Task:
Precondition: numbers list has elements removed, `num` contains the last element that was removed
Program Fragment:
```
if num > 1:
    total += num
...
else part: numbers list has elements removed, `num` contains the last element that was removed. The message 'Skipping non-positive number: num' is printed to the console.
