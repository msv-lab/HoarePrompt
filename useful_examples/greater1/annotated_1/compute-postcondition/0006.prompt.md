...
Your Task:
Precondition: `numbers` is a list of integers with one element removed, `total` is 0, `count` is 0, `num` is the popped integer
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with one element removed, `total` is the value of `num`, `count` is 1, `num` is the popped integer
else part: `numbers` is a list of integers with one element removed, `total` is 0, `count` is 0, `num` is the popped integer, and we print 'Skipping non-positive number: num'
