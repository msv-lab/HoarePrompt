...
Your Task:
Initial State: `numbers` is a list of integers with one less element, `total` is the sum of the removed value and previous total, `count` is 0, and `num` is the value removed from the list
```
count += 1
