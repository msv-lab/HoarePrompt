...
Your Task:
Precondition: `numbers` has one less element, `total`, `count`, and `num` are adjusted accordingly
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` has one less element, `total` is updated by adding `num`, `count` is increased by 1, `num` is adjusted accordingly
else part: `numbers` has one less element, `total`, `count`, and `num` are adjusted accordingly
