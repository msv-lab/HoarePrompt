...
Your Task:
Precondition: `numbers` is a list of integers with elements remaining, `total` and `count` are adjusted based on the last element of `num`
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with elements remaining, `total` and `count` are adjusted based on the last element of `num`, `count` is increased by 1
else part: `numbers` is a list of integers with elements remaining, `total` and `count` are adjusted based on the last element of `num`
