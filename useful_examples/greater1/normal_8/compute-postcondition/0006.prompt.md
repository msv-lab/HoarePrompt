...
Your Task:
Precondition: `numbers` is a list of integers with one less element, `total` is 0, `count` is 0, and `num` holds the popped value
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` is a list of integers with one less element, `total` is updated with the added value of `num`, `count` is 1, and `num` holds the popped value
else part: `numbers` is a list of integers with one less element, `total` is 0, `count` is 0, `num` holds the popped value
