...
Your Task:
Precondition: `numbers` list has one less element, `total` is 0, `count` is 0, and `num` has the value popped from the list
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` list has one less element, `total` is the value of `num`, `count` is 1, and `num` has been added to `total`
else part: `numbers` list has one less element, `total` is 0, `count` is 0, and `num` has been printed as a skipped non-positive number
