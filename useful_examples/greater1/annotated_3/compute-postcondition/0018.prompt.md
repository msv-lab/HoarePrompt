...
Your Task:
Precondition: `numbers` list has either one less element or remains the same, `total`, `count`, and `num` adjusted accordingly
Program Fragment:
```
if num > 1:
    total += num
...
```
if part: `numbers` list has either one less element or remains the same, `total` is increased by the value of `num`, `count` is adjusted accordingly, `count` is increased by 1
else part: `numbers` list has either one less element or remains the same, `total`, `count`, and `num` adjusted accordingly
