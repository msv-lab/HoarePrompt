...
Your Task:
State at the end of the previous iteration: `total` is updated based on the value at index [0][0] in the `matrix` array, `i` is 1, `n` is greater than 1, and the total is increased by the value at index [i][n - i - 1] in the `matrix` array
```
for i in range(n):
    # the loop body is omit
```
