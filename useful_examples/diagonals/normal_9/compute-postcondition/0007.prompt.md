...
Your Task:
Initial State: `matrix` is a square matrix of size n x n where n is a positive integer, `total` is the updated sum of all diagonal elements in the matrix, `i` is adjusted to `i+1`
```
