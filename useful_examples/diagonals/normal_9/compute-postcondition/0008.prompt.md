...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix of size n x n, `total` is the updated sum of all diagonal elements in the matrix, `i` is adjusted to `i+1
```
for i in range(n):
    # the loop body is omit
```
