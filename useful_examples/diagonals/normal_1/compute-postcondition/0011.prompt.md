...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix (n x n), `total` is the updated value after adding matrix[0][n - 1], matrix[1][n - 2], and matrix[2][n - 3] + matrix[2][2]
```
for i in range(n):
    # the loop body is omit
```
