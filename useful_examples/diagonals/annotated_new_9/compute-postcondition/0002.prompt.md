...
Your Task:
State before the loop starts: matrix is a square matrix (n x n) where n is a positive integer, `n` is the number of rows or columns in the matrix, `total` is 0
```
for i in range(n):
    # the loop body is omit
```
