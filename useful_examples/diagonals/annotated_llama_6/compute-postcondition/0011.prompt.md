...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers with at least 1 row and column, `n` is greater than 2, `total` is `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2] + matrix[2][2] + matrix[2][n - 3]`, `i` is 2
```
for i in range(n):
    # the loop body is omit
```
