...
#Overall this is what the function does: The function accepts a square matrix of size n x n, calculates the sum of diagonal elements, and returns the total sum along with the size of the matrix.

```

Return Postconditions: `total` is the sum of diagonal elements of `matrix`, `i` is `n`, `n` is the size of the matrix
