...
Your Task:
Initial State: `matrix` is a square matrix of dimension n x n, `total` is the sum of the diagonal elements up to the (i+1)th index, `i` is increased by 1, `n` is greater than `i`
```
total += matrix[i][i]
