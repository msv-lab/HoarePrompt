...
Your Task:
State before the loop starts: matrix is a square matrix of dimension n x n, where n is a non-negative integer, and `n` is equal to the number of rows or columns in the matrix; `total` is 0
```
for i in range(n):
    # the loop body is omit
```
