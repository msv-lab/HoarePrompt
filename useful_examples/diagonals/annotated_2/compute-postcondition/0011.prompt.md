...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix of dimension n x n, `total` is the sum of the diagonal elements up to the (i+1)th index, `i` is increased by 1, `n` is greater than `i`
```
for i in range(n):
    # the loop body is omit
```
