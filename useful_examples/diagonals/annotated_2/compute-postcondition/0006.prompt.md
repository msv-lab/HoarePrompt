...
Your Task:
Initial State: `matrix` is a square matrix of dimension n x n where n is a non-negative integer and n is greater than 0, `total` is the sum of the diagonal elements up to the (i+2)th index, `i` is increased by 1
```
total += matrix[i][i]
