...
Your Task:
State at the end of the previous iteration: `matrix` is a square list of lists of integers, `n` is greater than 0, `total` is the sum of `matrix[0][0]` and `matrix[0][n - 1]`, `i` is 0
```
for i in range(n):
    # the loop body is omit
```
