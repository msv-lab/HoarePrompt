...
Your Task:
Initial State: Precondition: matrix is a square matrix (n x n) where n is a non-negative integer. Each element of the matrix is a valid integer; `n` is the number of rows/columns in the square matrix.
```
total = 0
