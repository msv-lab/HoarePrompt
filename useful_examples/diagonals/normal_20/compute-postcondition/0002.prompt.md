...
Your Task:
State before the loop starts: matrix is a square matrix (n x n) where n is a non-negative integer; `n` is the number of rows/columns in the matrix, `total` is 0
```
for i in range(n):
    # the loop body is omit
```
