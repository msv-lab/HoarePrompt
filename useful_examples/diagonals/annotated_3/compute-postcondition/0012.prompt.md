...
Initial State: `matrix` is a square matrix (n x n), `n` is the number of rows/columns in the matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: `matrix` is a square matrix (n x n), `n` is greater than 0, `total` is the updated sum of the diagonal elements of the matrix
Output State after the loop executes 2 times: `matrix` is a square matrix (n x n), `n` is greater than 0, `total` is the updated sum of the diagonal elements of the matrix, `i` is less than `n+1`
Output State after the loop executes 3 times: `matrix` is a square matrix (n x n), `n` is greater than 0, `total` is the updated sum of the diagonal elements of the matrix + `matrix[n][n]`, `i` is equal to `n`
