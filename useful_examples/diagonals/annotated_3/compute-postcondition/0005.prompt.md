...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix (n x n), `n` is greater than 0, `total` is the updated sum of the diagonal elements of the matrix
```
for i in range(n):
    # the loop body is omit
```
