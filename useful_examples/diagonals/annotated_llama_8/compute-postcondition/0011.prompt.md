...
Your Task:
State at the end of the previous iteration: `matrix` is a 2D list of integers, `n` is greater than 2, `total` is incremented by the integer value of `matrix[2][n - 3]`, `i` is 2
```
for i in range(n):
    # the loop body is omit
```
