...
#Overall this is what the function does: The function accepts a square matrix and returns the sum of its main diagonal and anti-diagonal elements.

```

Return Postconditions: `n` is either greater than 0 or 0, `total` is either the sum of the elements on the main diagonal and the anti-diagonal or 0, `i` is either `n-1` or undefined, and the function returns `total`.
