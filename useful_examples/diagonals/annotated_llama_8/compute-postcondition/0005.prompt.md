...
Your Task:
State at the end of the previous iteration: `matrix` is a 2D list of integers, `n` is greater than 0, `total` is the sum of the top-left and top-right elements of `matrix`, `i` is 0
```
for i in range(n):
    # the loop body is omit
```
