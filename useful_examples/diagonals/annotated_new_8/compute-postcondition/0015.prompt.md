...
def func_1(matrix):
    n = len(matrix)
    #State of the program here: `matrix` is a square 2D list of integers with dimensions n x n, where n is a non-negative integer; `n` is the side length of the square matrix
    total = 0
    #State of the program here: `matrix` is a square 2D list of integers with dimensions n x n, where n is a non-negative integer; `n` is the side length of the square matrix, `total` is 0
    for i in range(n):
        total += matrix[i][i]
    ...
    return total
    #State of the program after the return statement: If `n` is 0, `total` is 0. If `n` is greater than 0, `total` contains the sum of the diagonal elements of the matrix.
#Overall this is what the function does: The function accepts a square 2D list of integers matrix with dimensions n x n. If n is greater than 0, it returns the sum of the diagonal elements of the matrix. If n is 0, it returns 0.

```

Return Postconditions: If `n` is 0, `total` is 0. If `n` is greater than 0, `total` contains the sum of the diagonal elements of the matrix.
