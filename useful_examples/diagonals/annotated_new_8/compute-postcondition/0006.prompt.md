...
Your Task:
Initial State: `matrix` is a square 2D list of integers with dimensions n x n, where n is a non-negative integer; `total` is updated with the value at index (0, 0) + matrix[0][n - 1], `i` is 1
```
total += matrix[i][i]
