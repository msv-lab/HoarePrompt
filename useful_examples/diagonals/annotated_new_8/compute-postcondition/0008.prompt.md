...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers with dimensions n x n, where n is a non-negative integer; `total` is updated with the new total value, `i` is incremented by 1
```
for i in range(n):
    # the loop body is omit
```
