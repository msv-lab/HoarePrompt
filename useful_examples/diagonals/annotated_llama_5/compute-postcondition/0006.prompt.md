...
Your Task:
Initial State: `matrix` is a square list of lists of integers with at least one row and one column, `n` is greater than 1, `total` is the sum of the top left and top right elements of `matrix`, `i` is 1
```
total += matrix[i][i]
