...
Your Task:
Initial State: `matrix` is a square list of lists of integers with at least one row and one column, `n` is greater than 2, `total` is the sum of the top left, top right, second diagonal, and the element at position `[1][n - 2]` in the `matrix`, `i` is 2
```
total += matrix[i][i]
