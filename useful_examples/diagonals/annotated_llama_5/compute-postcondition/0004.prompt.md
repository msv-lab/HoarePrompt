...
Your Task:
Initial State: `matrix` is a square list of lists of integers with at least one row and one column, `n` is greater than 0, `total` is the value of the top left element of `matrix`, `i` is 0
```
