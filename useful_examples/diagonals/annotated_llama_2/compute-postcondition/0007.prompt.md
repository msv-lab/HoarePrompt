...
Your Task:
Initial State: `matrix` is a square 2D list of integers with at least 1 inner list, `n` is greater than 1, `i` is 1, `total` is `matrix[1][0] + matrix[1][n - 1] + matrix[1][1]`.
```
