...
Your Task:
State at the end of the previous iteration: `matrix` is a 2D list representing a square matrix, `n` is greater than 0, `i` is 0, `total` is the initial value of `total` plus the value of `matrix[0][0]` and `matrix[0][n - 1]`.
```
for i in range(n):
    # the loop body is omit
```
