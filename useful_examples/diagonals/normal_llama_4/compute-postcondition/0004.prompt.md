...
Your Task:
Initial State: `matrix` is a 2D list representing a square matrix, `n` is greater than 0, `i` is 0, `total` is the initial value of `total` plus the value of `matrix[0][0]`.
```
