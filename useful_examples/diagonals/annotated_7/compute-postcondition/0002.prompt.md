...
Your Task:
State before the loop starts: `matrix` is a square 2D list of integers of dimension n x n, where n is a non-negative integer; `n` is the dimension of the square matrix, `total` is 0
```
for i in range(n):
    # the loop body is omit
```
