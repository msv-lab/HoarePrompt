...
Your Task:
State at the end of the previous iteration: `matrix` is a square 2D list of integers of dimension n x n, `n` is a non-negative integer, `total` is updated according to the mentioned formula
```
for i in range(n):
    # the loop body is omit
```
