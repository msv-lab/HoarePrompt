...
Your Task:
Initial State: `matrix` is a square 2D list of integers of dimension n x n, `n` is a non-negative integer, `total` is the sum of the diagonal elements up to index 0 plus `matrix[2][2]` plus `matrix[2][n - 2 - 1]`, `i` is 2
```
total += matrix[i][i]
