...
Initial State: matrix is a square matrix (n x n) where n is a non-negative integer, `n` is the number of rows/columns in the matrix, `total` is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: `matrix` is a square matrix (n x n) where n is a non-negative integer, `n` is greater than 0, `total` is the sum of diagonal elements in the matrix
Output State after the loop executes 2 times: total is updated based on the sum of diagonal elements in the matrix
Output State after the loop executes 3 times: `total` is updated based on the sum of diagonal elements in the matrix, `n` is greater than `i+1`
