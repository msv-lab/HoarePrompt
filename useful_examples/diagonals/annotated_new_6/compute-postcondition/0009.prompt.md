...
Your Task:
Initial State: `matrix` is a square matrix (n x n) where n is a non-negative integer, `total` is updated to include the diagonal element at index (0, n - i - 1), `i` is increased by 1
```
total += matrix[i][i]
