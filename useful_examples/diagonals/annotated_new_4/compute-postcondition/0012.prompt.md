...
Initial State: matrix is a square 2D list of integers with dimensions n x n, where n is a positive integer, total is 0
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
```
Output State after the loop executes 1 times: total is the value at matrix[0][n - 1], i is 0
Output State after the loop executes 2 times: total is the updated value after adding the new element from the matrix
Output State after the loop executes 3 times: `total` is the updated value after adding the new element from the matrix, `i` is incremented by 1, `n` remains unchanged
