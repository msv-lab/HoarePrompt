...
Your Task:
State at the end of the previous iteration: `matrix` is a square matrix of integers with at least 1 row (and column), `n` is greater than 2, `total` is the sum of the values at positions `[0][0]`, `[0][n - 1]`, `[1][n - 2]` counted three times, `[1][1]`, `[2][2]`, and `[2][n - 3]` in the `matrix`, `i` is 2
```
for i in range(n):
    # the loop body is omit
```
