...
Your Task:
State at the end of the previous iteration: `total` is updated with the sum of `matrix[0][0]`, `matrix[0][n-1]`, `matrix[1][1]`, `matrix[1][n-2]`, `matrix[2][2]`, `matrix[2][n-3]`; `i` is 2; `n` is greater than 0
```
for i in range(n):
    # the loop body is omit
```
