...
def func_1(matrix):
    n = len(matrix)
    #State of the program here: matrix is a square matrix (n x n) where n is a positive integer. n is the size of the matrix.
    total = 0
    ...
    #State of the program after the  for loop has been executed: The loop calculates the sum of the values at the diagonal and antidiagonal of the square matrix `matrix` and adds them to `total`. The loop will execute for all rows in the matrix. If `n` is 0, the loop will not execute and `total` will remain 0.
    return total
    #State of the program after the return statement: `total` is the sum of the values at the diagonal and antidiagonal of the square matrix `matrix` if `n` is not 0, otherwise `total` is 0
#Overall this is what the function does: The function accepts a square matrix matrix, calculates the sum of values at the diagonal and antidiagonal if n is not 0, otherwise returns 0.

```

Return Postconditions: `total` is the sum of the values at the diagonal and antidiagonal of the square matrix `matrix` if `n` is not 0, otherwise `total` is 0
