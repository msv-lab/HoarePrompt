...
Your Task:
Initial State: `matrix` is a square 2D list of integers with dimensions n x n, `n` is the side length of the square matrix, `total` is 0, `i` is 0, `n` is greater than 0
```
total += matrix[i][i]
