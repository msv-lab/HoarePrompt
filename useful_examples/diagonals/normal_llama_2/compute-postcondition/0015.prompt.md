...
def func_1(matrix):
    n = len(matrix)
    #State of the program here: `matrix` is a square list of lists of integers, `n` is the length of `matrix`.
    total = 0
    #State of the program here: `matrix` is a square list of lists of integers, `n` is the length of `matrix`, `total` is 0
    for i in range(n):
        total += matrix[i][i]
        
        total += matrix[i][n - i - 1]
        
    #State of the program after the  for loop has been executed: If `n` is at least 1, then `total` is the sum of the elements on the main diagonal and the anti-diagonal of the matrix, `i` is `n-1`, and `matrix` is a square list of lists of integers. If `n` is 0, the loop does not execute, and `total` is 0.
    return total
    #State of the program after the return statement: If `n` is at least 1, then `total` is the sum of the elements on the main diagonal and the anti-diagonal of the matrix, `i` is `n-1`, and the function returns `total`. If `n` is 0, `total` is 0, and the function returns 0.
#Overall this is what the function does: The function accepts a square 2D list of integers and returns the sum of the elements on the main diagonal and the anti-diagonal if the matrix is not empty; otherwise, it returns 0.

```

Return Postconditions: If `n` is at least 1, then `total` is the sum of the elements on the main diagonal and the anti-diagonal of the matrix, `i` is `n-1`, and the function returns `total`. If `n` is 0, `total` is 0, and the function returns 0.
