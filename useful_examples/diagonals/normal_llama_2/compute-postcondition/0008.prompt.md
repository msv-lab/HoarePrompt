...
Your Task:
State at the end of the previous iteration: `matrix` is a square list of lists of integers, `n` is at least 2, `total` is increased by the value of `matrix[1][n-2]`, `i` is 1
```
for i in range(n):
    # the loop body is omit
```
