...
Your Task:
Initial State: `matrix` is a square list of lists of integers, `n` is greater than 1, `total` is increased by the value of `matrix[1][n-2]` plus `matrix[1][1]`, and `i` is 1
```
