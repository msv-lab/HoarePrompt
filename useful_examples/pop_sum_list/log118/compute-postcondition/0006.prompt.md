...
Your Task:
State at the end of the previous iteration: *`input_list` contains at least one integer less than before, `total_sum` is equal to `element`, and `element` is a positive integer greater than 0.
```
while input_list:
    # the loop body is omit
```
