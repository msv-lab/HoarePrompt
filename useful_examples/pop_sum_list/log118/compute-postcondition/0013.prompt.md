...
Your Task:
Precondition: `input_list` has at least one integer less, `element` is the last integer from `input_list`, `total_sum` is equal to the previous value plus `element`.
Program Fragment:
```
if element > 0:

```
if part: `input_list` has at least one integer, `element` is the last integer from `input_list`, `total_sum` is equal to the previous value plus 2 times `element`.
else part: None
