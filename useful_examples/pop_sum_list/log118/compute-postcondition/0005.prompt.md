...
Your Task:
Precondition: `input_list` contains at least one integer less than before, `total_sum` is 0, `element` is the last integer from the original `input_list`.
Program Fragment:
```
if element > 0:

```
if part: `input_list` contains at least one integer less than before, `total_sum` is equal to `element`, `element` is a positive integer greater than 0.
else part: None
