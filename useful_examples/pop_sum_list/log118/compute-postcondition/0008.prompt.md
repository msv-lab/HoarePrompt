...
Your Task:
Initial State: `input_list` has at least one integer remaining; `total_sum` is equal to `element`, and `element` is a positive integer greater than 0, now equal to the last integer popped from `input_list`.
```
total_sum += element
