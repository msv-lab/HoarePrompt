...
Your Task:
Initial State: `input_list` has at least one integer remaining, `element` is a positive integer greater than 0, and `total_sum` is equal to the previous value plus `element`.
```
