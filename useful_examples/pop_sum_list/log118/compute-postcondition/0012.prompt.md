...
Your Task:
Initial State: `input_list` has at least one integer less, `element` is the last integer from `input_list`, `total_sum` is equal to the previous value plus `element`.
```
total_sum += element
