...
Your Task:
State at the end of the previous iteration: *`input_list` has at least one integer remaining, `element` is a positive integer greater than 0, and `total_sum` is equal to the previous value plus `element`.
```
while input_list:
    # the loop body is omit
```
