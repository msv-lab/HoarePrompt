...
Your Task:
Initial State: *`input_list` contains at least one integer less than before, `total_sum` is 0, `element` is the last integer from the original `input_list`, and the current value of `element` is greater than 0.
```
total_sum += element
