...
Your Task:
State at the end of the previous iteration: `matrix` is a 2D list representing a square matrix of dimensions n×n; `n` is greater than 1; `total` is equal to `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2]; `i` is 1.
```
for i in range(n):
    # the loop body is omit
```
