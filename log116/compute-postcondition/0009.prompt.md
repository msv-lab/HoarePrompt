...
Your Task:
Initial State: `matrix` is a 2D list representing a square matrix of dimensions n×n; `n` must be greater than 2; `total` is equal to `matrix[0][0] + matrix[0][n - 1] + matrix[1][1] + matrix[1][n - 2]; `i` is 2
```
total += matrix[i][i]
