...
Your Task:
State before the loop starts: `matrix` is a 2D list representing a square matrix of dimensions n×n; `n` is the length of `matrix`, which is a positive integer; `total` is 0.
```
for i in range(n):
    # the loop body is omit
```
