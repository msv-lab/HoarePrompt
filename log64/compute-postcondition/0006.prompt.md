...
Your Task:
Precondition: *`x` is an integer, `x` is less than or equal to 0, and `x` is less than 0.
Program Fragment:
```
if x < -10:
    x -= 10
...
```
if part: `x` is an integer, `x` is less than or equal to 0, `x` is less than 0, `x` is less than -10, and `x` is now decreased by 10.
else part: None
