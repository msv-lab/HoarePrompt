...
Your Task:
Initial State: `total` is equal to the sum of all elements from the main diagonal and the anti-diagonal of `matrix`, `n` is a non-negative integer, if `n` is greater than 0 then `total` is the sum involving `n` elements, else `total` is 0.
```
return total
