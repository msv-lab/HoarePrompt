...
Initial State: `matrix` is a 2D list (square matrix) of integers; `n` is the number of rows (or columns) in `matrix`; `total` is 0.
Code of the loop:
```
for i in range(n):
    total += matrix[i][i]
...
Output State after the loop executes 1 times: `total` is equal to `matrix[0][0] + matrix[0][n - 1]`, `n` must be greater than or equal to 1, `i` is 0
