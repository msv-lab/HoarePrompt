...
#Overall this is what the function does: The function accepts a parameter x, returns x minus 1 if x is less than 0; returns x plus 1 if x is greater than 0; returns 0 if x is equal to 0.

```

Return Postconditions: Case_1: The program returns x minus 1, where x is an integer less than 0

Case_2: The program returns the value of x, which is either x + 1 if x > 0, x - 1 if x < 0, or 0 if x = 0.
