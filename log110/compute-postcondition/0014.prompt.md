...
Your Task:
State at the end of the previous iteration: `n` is a non-negative integer such that 0 <= n < 10^k; `k` is at least 2; `dp[n, k]` is updated to `9 * 10 + 90 * (10^{(k - 2)} - 10)`; `i` is 2.
```
for i in range(1, k):
    # the loop body is omit
```
