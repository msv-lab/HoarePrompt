...
```
Output State after the loop executes 1 times: `total` is `num`, where `num` is an integer from the first row of the matrix
Output State after the loop executes 2 times: `total` is `2 * num`, `num` remains unchanged
Output State after the loop executes 3 times: `total` is `2 * num + num`, `num` remains unchanged
