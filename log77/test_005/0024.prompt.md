...
Your Task:
Initial State: `total` is the updated sum of all integers in `row`, `num` is the fourth integer in `row`, for the loop to execute a fourth time `row` must have at least 4 integers.
```
total += num
