...
Your Task:
Initial State: `total` is `2 * num`, `num` remains unchanged, for the loop to execute a third time `row` must have at least 3 elements, and `num` will be the next value in `row`
```
total += num
