...
Your Task:
State before the loop starts: `total` is the sum of all integers in `row`, `num` is the last integer in `row`, for the loop to execute a second time `matrix` must have at least one more row than it had at the end of the last iteration.
```
for num in row:
    # the loop body is omit
```
