...
Your Task:
Initial State: `total` is the sum of all integers in the last processed row, for the loop to execute the first time `row` is a list of integers with at least 1 integer, `num` is the first integer in `row`
```
total += num
