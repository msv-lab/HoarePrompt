...
Your Task:
State before the loop starts: `total` is the sum of all integers in the last processed row, `num` is the last integer in that row, `row` is the next row in the matrix.
```
for num in row:
    # the loop body is omit
```
