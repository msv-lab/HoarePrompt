...
Initial State: `total` is the sum of all integers in `row`, `num` is the last integer in `row`, for the loop to execute a second time `matrix` must have at least one more row than it had at the end of the last iteration.
Code of the loop:
```
for num in row:
    total += num
...
```
Output State after the loop executes 1 times: `total` is the updated sum of all integers in `row`, `num` is the second integer in `row
Output State after the loop executes 2 times: `total` is the updated sum of all integers in `row`, `num` is the third integer in `row`, and `total` is updated to `total + num`.
Output State after the loop executes 3 times: `total` is updated by adding `num` to it, where `num` is the fourth integer in `row`
