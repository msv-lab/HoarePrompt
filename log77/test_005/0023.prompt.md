...
Your Task:
State at the end of the previous iteration: `total` is the updated sum of all integers in `row`, `num` is the third integer in `row`, and `total` is updated to `total + num`.
```
for num in row:
    # the loop body is omit
```
