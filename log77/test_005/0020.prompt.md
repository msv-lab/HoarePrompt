...
Your Task:
Initial State: `total` is the sum of all integers in `row`, `num` is the second integer in `row`, for the loop to execute a second time `row` must have at least 2 integers.
```
total += num
