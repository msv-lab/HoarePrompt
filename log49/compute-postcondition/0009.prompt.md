...
#Overall this is what the function does: The function accepts a non-negative integer n, returns n if n is greater than 1. If n is a non-negative integer less than or equal to 1, it returns the sum of func_1(0) and func_1(n-2).

```

Return Postconditions: Case_1: The program returns the non-negative integer 'n' that is greater than 1

Case_2: The program returns the value obtained by adding the value returned by func_1(0) and the value returned by func_1(n-2), where n is a non-negative integer less than or equal to 1
