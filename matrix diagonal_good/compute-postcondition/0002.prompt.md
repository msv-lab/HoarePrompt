...
Your Task:
State before the loop starts: `matrix` is a 2D list representing a square matrix of dimensions `n x n`, where `n` is a positive integer equal to `len(matrix)`; `total` is 0.
```
for i in range(n):
    # the loop body is omit
```
