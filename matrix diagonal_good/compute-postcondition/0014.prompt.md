...
#Overall this is what the function does: The function accepts a 2D list matrix representing a square matrix and returns the total sum of all diagonal elements.

```

Return Postconditions: The program returns total, which is equal to the sum of all diagonal elements of the matrix, including elements matrix[i][i] and matrix[i][n - i - 1] for all i from 0 to n-1.
