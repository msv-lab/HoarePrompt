...
Your Task:
Initial State: `numbers` is a list of integers with the last element removed and it is not empty, `num` is the removed integer, `total` and `count` may or may not have been updated based on the value of `num`.
```
num = numbers.pop()
